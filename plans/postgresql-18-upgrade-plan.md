...
4. **Scope is the shipped product**: `mailwave`, `deploy` and `docs` repos. `cloud/` is out of scope (it already runs PG18 — see §3.5).
...
| PG18 incompatibility | Mailwave | Evidence |
|---|---|---|
...
| External PostgreSQL (`DB_HOST` → RDS/Cloud SQL/…) | Nothing. Mailwave only connects. | Nothing — 18 already works |
| Coolify one-click (`deploy/coolify/mailwave.yaml`) | Same crash loop. Coolify's `getMountPath` runs on **create only**, so an existing service keeps `/var/lib/postgresql/data` | Update the template + a doc note |
...
So a Mailwave user cannot `docker compose -f compose.alloydb.yaml up -d` on PG18 without registering with Google. **`compose.alloydb.yaml` can move 17.5.0 → 17.9.0 and no further.**
...
It also runs `reindexdb --all --concurrently` and a per-database `VACUUM (ANALYZE)` afterwards, serially, with no `-j`. Over one database per workspace that entirely negates `--link`'s speed. **`PGAUTO_REINDEX=no` is mandatory for this to be usable at Mailwave's shape** — and it is safe, per §3.3.
...
| `compose.yaml:50-51` | `depends_on: - postgres` → `depends_on: postgres: condition: service_healthy` (matches `deploy/coolify/mailwave.yaml:34-36`, and stops the api racing a freshly-initdb'd cluster) |
...
This is the highest-value code change in the plan. Today, pointed at an empty cluster, Mailwave manufactures a healthy-looking install in about a second, with one `info` line among ~100 startup lines as the only signal — reproduced against the real binary:
...
A related hole: if the system DB is restored but a workspace DB is missing, and PostgreSQL was upgraded *without* upgrading Mailwave, the app boots clean and then `pkg/database/connection_manager.go:291` catches SQLSTATE `3D000` on first access and **silently** `CREATE DATABASE`s an empty workspace — minutes after the upgrade, decoupled from it. (If Mailwave *was* also upgraded, migration fails closed and the app refuses to start — correct behaviour.)
...
| `deploy/coolify/mailwave.yaml:43,49` + `:39-42` | Image, volume path, comment — same three edits as `compose.yaml` |
...
> - **Improvement**: The bundled PostgreSQL moves to 18 for new installations, and the compose file now mounts its volume at `/var/lib/postgresql` as PostgreSQL 18's image requires. Mailwave still runs on PostgreSQL 17 and above, so nothing forces you to move. Existing installations must upgrade the data directory before starting the new container — one command, documented under Self-hosting → Upgrading PostgreSQL. Back up first: the upgrade is done in place and is not reversible.

**Out of this repo**: the Dokploy catalog entry (`Dokploy/templates`, `blueprints/mailwave/`) is maintained upstream and needs its own PR. Note that Dokploy mounts the PGDATA leaf `/var/lib/postgresql/18/docker` rather than the parent, which forfeits `pg_upgrade --link` for every future major — worth raising with them.
...
# 2. Stop the app cleanly. -t 75 matters: Mailwave asks for 65s to drain its
...
#    Mailwave's system DB references per-workspace DBs across that boundary.
docker run --rm --network mailwave_mailwave-network -v "$PWD/backup:/backup" postgres:18-alpine \
...
for db in $(docker compose exec -T postgres psql -U postgres -Atc \
     "SELECT datname FROM pg_database WHERE datname='mailwave_system' OR datname LIKE 'mailwave\_ws\_%'"); do
  docker run --rm --network mailwave_mailwave-network -v "$PWD/backup:/backup" postgres:18-alpine \
...
docker run --rm -v mailwave_postgres-data:/v -v "$PWD/backup:/backup" alpine \
  tar czf /backup/pg17-volume.tgz -C /v .

# 5. Update Mailwave to the new release, but do NOT start it yet.
...
- **Backups miss every workspace database.** `cloud/manager/internal/backup/pgdump.go:27` dumps only `mailwave_system`; `grep mailwave_ws` over `cloud/manager/internal/backup/` returns nothing. `backup-scripts/backup_postgres_to_gcs.sh:31` defaults to a single database too. Contacts, message history and all web-analytics partitions live in the per-workspace databases. This is the most serious thing the audit found and it has nothing to do with PostgreSQL 18.
