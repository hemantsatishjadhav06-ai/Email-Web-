...
# Default values
SMTP_SERVER="${SMTP_SERVER:-localapi.mailwave.com}"
...
if [ -z "$API_EMAIL" ] || [ -z "$API_KEY" ] || [ -z "$WORKSPACE_ID" ]; then
    ...
    echo "  SMTP_SERVER=localapi.mailwave.com    # SMTP server address"
...
elif [ -f "./dev-certs/localapi.mailwave.com.cert.pem" ]; then
    TLS_CA_PATH="--tls-ca-path ./dev-certs/localapi.mailwave.com.cert.pem"
    print_info "Using TLS certificate: ./dev-certs/localapi.mailwave.com.cert.pem"
else
