...
**Example:**
```bash
./scripts/generate-dev-certs.sh localapi.mailwave.com
```
...
```bash
SMTP_SERVER=localapi.mailwave.com    # SMTP server address
...
```bash
echo "127.0.0.1 localapi.mailwave.com" | sudo tee -a /etc/hosts
```
...
```bash
--tls-ca-path ./dev-certs/localapi.mailwave.com.cert.pem
```
