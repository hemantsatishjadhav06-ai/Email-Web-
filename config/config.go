...
import (
	...
	"github.com/Mailwave/mailwave/pkg/smtp_bridge"
...
type Config struct {
	...
	// LicenseKey is the raw MAILWAVE_LICENSE_KEY envelope, unverified. It is carried
	...
	//
	// The name carries the MAILWAVE_ prefix that no other variable here does, deliberately:
	// it pairs with MAILWAVE_LICENSE_SIGNING_KEY, which the licensegen CLI in the private cloud repository reads, and a bare
...
// PlanLimitsConfig holds the quotas of the subscribed plan. Mailwave Cloud sets
...
func LoadWithOptions(opts LoadOptions) (*Config, error) {
	...
	v.SetDefault("DB_PREFIX", "mailwave")
	v.SetDefault("DB_NAME", "mailwave_system")
	...
	// SMTP defaults
	v.SetDefault("SMTP_FROM_NAME", "Mailwave")
	...
	v.SetDefault("TRACING_SERVICE_NAME", "mailwave-api")
	...
	if isInstalled && systemSettings != nil {
		...
		if smtpConfig.FromName == "" {
			smtpConfig.FromName = "Mailwave" // Default
		}
	...
	// Plan limits, as pushed by the Mailwave Cloud control plane. Unset means 0,
	...
	config := &Config{
		...
		LicenseKey: v.GetString("MAILWAVE_LICENSE_KEY"),
