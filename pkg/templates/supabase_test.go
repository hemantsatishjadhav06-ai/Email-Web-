...
func TestSupabaseTemplateCreation(t *testing.T) {
	t.Run("All Supabase templates can be created", func(t *testing.T) {
		...
		for name, createFunc := range templates {
			t.Run(name, func(t *testing.T) {
				...
				assert.Equal(t, mailwave_mjml.MJMLComponentMjml, block.GetType(),
	...
	t.Run("Signup template has correct structure", func(t *testing.T) {
		...
		// Check head
		head := children[0]
		assert.Equal(t, mailwave_mjml.MJMLComponentMjHead, head.GetType())

		// Check body
		body := children[1]
		assert.Equal(t, mailwave_mjml.MJMLComponentMjBody, body.GetType())
	...
	t.Run("All templates can be marshaled and unmarshaled", func(t *testing.T) {
		...
		for name, createFunc := range templates {
			t.Run(name, func(t *testing.T) {
				...
				// Unmarshal back
				retrieved, err := mailwave_mjml.UnmarshalEmailBlock(originalJSON)
...
func TestSupabaseTemplateComponents(t *testing.T) {
	t.Run("Signup template uses all required components", func(t *testing.T) {
		...
		expectedComponents := []mailwave_mjml.MJMLComponentType{
			mailwave_mjml.MJMLComponentMjml,
			mailwave_mjml.MJMLComponentMjHead,
			mailwave_mjml.MJMLComponentMjBody,
			mailwave_mjml.MJMLComponentMjWrapper,
			mailwave_mjml.MJMLComponentMjSection,
			mailwave_mjml.MJMLComponentMjColumn,
			mailwave_mjml.MJMLComponentMjImage,
			mailwave_mjml.MJMLComponentMjText,
			mailwave_mjml.MJMLComponentMjButton,
		}
...
func findAllComponentTypes(block mailwave_mjml.EmailBlock) []mailwave_mjml.MJMLComponentType {
	if block == nil {
		return nil
	}

	types := []mailwave_mjml.MJMLComponentType{block.GetType()}
