...
func TestSMTPMailer_SendWorkspaceInvitation(t *testing.T) {
	...
	inviterName := "Test Inviter"
	token := "test-token-123"
	baseURL := "https://mailwave.example.com"
...
func TestNewSMTPMailer(t *testing.T) {
	...
	config := &Config{
		...
		FromEmail:    "noreply@example.com",
		FromName:     "Mailwave",
		APIEndpoint:  "https://mailwave.example.com",
	}
...
func TestSMTPMailer_LocalizedSubjects(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			expectedSubject: "Subject: Votre code d'authentification Mailwave",
		},
		{
			...
			expectedSubject: "Subject: Vous avez été invité à rejoindre Mon Espace sur Mailwave",
		},
		{
...
func TestSMTPMailer_UnknownLanguageFallsBackToEnglish(t *testing.T) {
	...
	if !strings.Contains(logOutput, "Subject: Your Mailwave authentication code") {
...
func TestConsoleMailer_LocalizedContent(t *testing.T) {
	...
	expectedStrings := []string{
		...
		"Bonjour,",
		"Votre code d'authentification pour Mailwave est :",
		"123456",
		"L'équipe Mailwave",
	}
	for _, expected := range expectedStrings {
