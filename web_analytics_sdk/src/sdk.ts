...
export class MailwaveAnalyticsSDK {
   ...
   */
  init(userConfig: MailwaveAnalyticsConfig): Promise<void> {
   ...
   */
  private async initializeAsync(userConfig: MailwaveAnalyticsConfig): Promise<void> {
  ...
  private checkAndUpdateMaxDuration(): boolean {
    ...
    if (totalActiveMs >= this.config.heartbeatMaxDuration) {
      ...
      if (this.config.debug) {
        const tierResult = this.getCurrentTier();
        console.log(
          `[MailwaveAnalytics] Heartbeat max duration reached ` +
   ...
   */
  getConfig(): Readonly<MailwaveAnalyticsConfig> | null {
    if (!this.config) return null;
    return { ...this.config };
  }
