 ...
 */
const BUNDLE = path.resolve(__dirname, '../dist/mailwave-analytics.min.js');
...
describe('shipped bundle', () => {
  ...
  it('exposes the API on window, not the module namespace', () => {
    ...
    // top-level `var MailwaveAnalytics` at real script scope overwriting the
    // global with rollup's module-namespace object. Function scope hides it,
    // because `var` never reaches window there.
    window.eval(code);

    const global = (window as unknown as Record<string, unknown>).MailwaveAnalytics as
      | Record<string, unknown>
      | undefined;

    expect(global, 'the bundle must define window.MailwaveAnalytics').toBeDefined();
