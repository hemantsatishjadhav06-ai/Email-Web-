...
import { MailwaveAnalyticsSDK } from './sdk';
import type {
  MailwaveAnalyticsConfig,
  MailwaveAnalyticsAPI,
  GoalData,
  SessionDebugInfo,
} from './types';
...
const MailwaveAnalytics: MailwaveAnalyticsAPI = {
  init: (config: MailwaveAnalyticsConfig) => sdk.init(config),
...
type InstallHost = Record<string, MailwaveAnalyticsAPI | undefined>;
...
if (!alreadyInstalled) {
  ...
  console.warn(
    '[MailwaveAnalytics] SDK already loaded on this page; ignoring the duplicate ' +
...
export default alreadyInstalled ?? MailwaveAnalytics;
