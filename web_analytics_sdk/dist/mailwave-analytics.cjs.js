...
class SessionManager {
    ...
    getOrCreateSession() {
        ...
        if (stored && !this.isSessionExpired(stored)) {
            ...
            this.saveSession();
            if (this.debug) {
                console.log('[MailwaveAnalytics] Resumed session:', stored.id);
            }
            return stored;
        }
        // Create new session
    ...
    isValidCrossDomain() {
        ...
        if (age > expiry) {
            if (this.debug) {
                console.log('[MailwaveAnalytics] Cross-domain input expired:', age, 'seconds old');
            }
            return false;
        }
        ...
        if (timestamp > now + CLOCK_SKEW_TOLERANCE$1) {
            if (this.debug) {
                console.log('[MailwaveAnalytics] Cross-domain timestamp too far in future');
            }
            return false;
        }
        return true;
    ...
    createSessionFromCrossDomain() {
        ...
        this.saveSession();
        if (this.debug) {
            console.log('[MailwaveAnalytics] Created session from cross-domain:', session.id);
        }
        return session;
    }
    /**
    ...
    createSession() {
        ...
        this.saveSession();
        if (this.debug) {
            console.log('[MailwaveAnalytics] Created session:', session.id);
        }
        return session;
    }
    /**
    ...
    setDimension(index, value) {
        ...
        this.saveSession();
        if (this.debug) {
            console.log(`[MailwaveAnalytics] Set dimension custom_${index}:`, value);
        }
    }
    /**
    ...
    setIdentity(identity) {
        ...
        if (identity.token && utf8Length(identity.token) > MAX_IDENTITY_TOKEN_BYTES) {
            if (this.debug) {
                console.warn('[MailwaveAnalytics] Ignored an oversized identify token');
            }
            return;
        }
        if (!this.session)
        ...
        this.saveSession();
        if (this.debug) {
            console.log('[MailwaveAnalytics] Identity set');
        }
    }
    getIdentity() {
        if (!this.session)
    ...
    applyUrlDimensions(urlDimensions) {
        ...
        for (const [index, value] of Object.entries(urlDimensions)) {
            ...
            if (!this.session.dimensions[numIndex]) {
                ...
                changed = true;
                if (this.debug) {
                    console.log(`[MailwaveAnalytics] Set dimension custom_${numIndex} from URL:`, value);
                }
            }
        }
        if (changed) {
...
class Sender {
    ...
    async handleOnline() {
        if (this.debug) {
            console.log('[MailwaveAnalytics] Back online, flushing queue');
        }
    ...
    async sendSession(payload) {
        ...
        if (this.debug) {
            console.log('[MailwaveAnalytics] Sending session payload:', payload);
        }
        ...
        if (result.outcome === 'ok') {
            if (this.debug) {
                console.log('[MailwaveAnalytics] Session response:', result.data);
            }
            return { success: true };
        }
        if (this.debug) {
            console.error('[MailwaveAnalytics] Send failed:', result.error);
        }
        return {
            success: false,
            error: result.error,
    ...
    sendSessionBeacon(payload) {
        ...
        if (this.debug) {
            console.log('[MailwaveAnalytics] Sending session beacon:', payload);
        }
        ...
        if (typeof fetchLater === 'function') {
            try {
                ...
                if (this.debug) {
                    console.log('[MailwaveAnalytics] Session queued via fetchLater');
                }
                return true;
            }
            catch {
        ...
        if (useBeacon && navigator.sendBeacon) {
            try {
                ...
                if (success) {
                    if (this.debug) {
                        console.log('[MailwaveAnalytics] Session sent via beacon');
                    }
                    return true;
                }
            }
        ...
        try {
            ...
            if (pending && typeof pending.catch === 'function') {
                pending.catch(() => {
                    if (this.debug) {
                        console.warn('[MailwaveAnalytics] keepalive fetch rejected; beat stays queued');
                    }
                });
            }
            if (this.debug) {
                console.log('[MailwaveAnalytics] Session sent via fetch keepalive');
            }
            return true;
        }
        catch {
...
class CrossDomainLinker {
    ...
    start() {
        ...
        if (this.config.debug) {
            console.log('[MailwaveAnalytics] CrossDomainLinker started', {
    ...
    stop() {
        ...
        if (this.config.debug) {
            console.log('[MailwaveAnalytics] CrossDomainLinker stopped');
        }
    }
    /**
    ...
    handleClick(e) {
        ...
        if (decorated !== target.href) {
            ...
            if (this.config.debug) {
                console.log('[MailwaveAnalytics] Decorated link:', decorated);
            }
        }
    }
    /**
    ...
    handleSubmit(e) {
        ...
        if (this.config.debug) {
            console.log('[MailwaveAnalytics] Decorated form:', form.action);
        }
    }
    /**
...
class MailwaveAnalyticsSDK {
    constructor() {
        ...
        this.onPageShow = (event) => {
            if (event.persisted) {
                ...
                this.resumeTracking();
                if (this.config?.debug) {
                    console.log('[MailwaveAnalytics] Restored from bfcache');
                }
            }
        };
    }
    ...
    async initializeAsync(userConfig) {
        ...
        // Check for bots
        if (isBot()) {
            console.log('[MailwaveAnalytics] Bot detected, tracking disabled');
            return;
        }
        // Merge config
        this.config = {
        ...
        if (this.config.debug) {
            console.log('[MailwaveAnalytics] Initialized', {
                session_id: session.id,
    ...
    onNavigation(url) {
        if (!this.sessionState)
            return;
        if (this.config?.debug) {
            console.log('[MailwaveAnalytics] Navigation:', url);
        }
    ...
    startHeartbeat() {
        ...
        if (this.heartbeatState.maxDurationReached) {
            if (this.config.debug) {
                console.log('[MailwaveAnalytics] Heartbeat not started: max duration reached');
            }
            return;
        }
    ...
    scheduleNextHeartbeat() {
        ...
        if (interval === null) {
            ...
            this.stopHeartbeat(true);
            if (this.config.debug) {
                console.log('[MailwaveAnalytics] Heartbeat stopped by tier configuration');
            }
            return;
        }
        ...
        this.heartbeatTimeout = setTimeout(() => {
            ...
            if (this.shouldSendPing()) {
                ...
                if (drift > 1000 && this.config?.debug) {
                    console.warn(`[MailwaveAnalytics] Heartbeat drift: ${drift}ms`);
                }
    ...
    checkAndUpdateMaxDuration() {
        ...
        if (totalActiveMs >= this.config.heartbeatMaxDuration) {
            ...
            if (this.config.debug) {
                const tierResult = this.getCurrentTier();
                console.log(`[MailwaveAnalytics] Heartbeat max duration reached ` +
    ...
    async ensureInitialized() {
        if (this.isInitialized)
            return;
        if (!this.initPromise) {
            throw new Error('Mailwave Analytics not configured. Set window.MailwaveAnalyticsConfig before loading the SDK.');
        }
 ...
 * UMD wrapper assigns window.MailwaveAnalytics from this default export, so a
 ...
 */
const INSTALL_KEY = '__mailwaveAnalytics';
const host = (typeof window !== 'undefined' ? window : undefined);
const alreadyInstalled = host?.[INSTALL_KEY];
if (!alreadyInstalled) {
    if (host) {
        host[INSTALL_KEY] = MailwaveAnalytics;
    }
    // Auto-initialize from global config
    if (typeof window !== 'undefined' && window.MailwaveAnalyticsConfig) {
        sdk.init(window.MailwaveAnalyticsConfig);
    }
}
else {
    console.warn('[MailwaveAnalytics] SDK already loaded on this page; ignoring the duplicate ' +
        'install. Remove the extra script tag — two copies would corrupt each ' +
        "other's session data.");
}
// Default export for UMD/ESM/CJS
var index = alreadyInstalled ?? MailwaveAnalytics;

module.exports = index;
//# sourceMappingURL=mailwave-analytics.cjs.js.map
