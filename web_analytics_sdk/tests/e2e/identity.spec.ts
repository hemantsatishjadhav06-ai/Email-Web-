...
test.describe('Identity', () => {
  ...
  test('getIdentity() returns null initially', async ({ page }) => {
    ...
    const identity = await page.evaluate(() => MailwaveAnalytics.getIdentity());
  ...
  test('identify() stores the email and its hmac verbatim', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.identify('User_123@Example.com', 'sig_abc'));
  ...
  test('identify() rejects an unsigned address', async ({ page }) => {
    ...
    const error = await page.evaluate(async () => {
      try {
        ...
        await MailwaveAnalytics.identify('no_hmac@example.com');
        return null;
      } catch (err) {
  ...
  test('identity persists after page reload', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.identify('reload_user@example.com', 'sig_reload'));
  ...
  test('identity is read back from its own key, not from the session blob', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.identify('durable@example.com', 'sig_durable'));
  ...
  test('identity is exposed on the debug session', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.identify('debug_user@example.com', 'sig_debug'));

    // debug() carries the whole Session; the identity rides on it, there is no
    // top-level user id any more.
    const session = await page.evaluate(() => MailwaveAnalytics.debug().session);
  ...
  test('clearIdentity() drops the identity and its storage key', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.identify('temp_user@example.com', 'sig_temp'));
    expect(await page.evaluate(() => MailwaveAnalytics.getIdentity())).toEqual({
      email: 'temp_user@example.com',
      hmac: 'sig_temp',
    });

    await page.evaluate(() => MailwaveAnalytics.clearIdentity());

    expect(await page.evaluate(() => MailwaveAnalytics.getIdentity())).toBeNull();
  ...
  test('identity does not survive a reload after clearIdentity()', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.identify('logout@example.com', 'sig_logout'));
    await page.evaluate(() => MailwaveAnalytics.clearIdentity());
  ...
  test('reset() clears identity', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.identify('will_reset@example.com', 'sig_reset'));
    expect(await page.evaluate(() => MailwaveAnalytics.getIdentity())).toEqual({
    ...
    await page.evaluate(() => MailwaveAnalytics.reset());
