...
test.describe('Network Handling', () => {
  ...
  test('falls back to fetch when beacon fails', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'fetch_test', type: 'other' }));
    await page.waitForTimeout(500);
  ...
  test('handles slow network gracefully', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'slow_test', type: 'other' }));
  ...
  test('handles network timeout gracefully', async ({ page }) => {
    ...
    const sessionId = await page.evaluate(() => MailwaveAnalytics.getSessionId());
  ...
  test('server returns checkpoint on success', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'goal1', type: 'other' }));
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'goal2', type: 'other' }));
    await page.waitForTimeout(500);
  ...
  test('continues sending after server failure', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'after_failure', type: 'other' }));
    await page.waitForTimeout(500);
  ...
  test('actions array grows cumulatively', async ({ page }) => {
    ...
    // Track several goals
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'goal_a', type: 'other' }));
    await page.waitForTimeout(200);
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'goal_b', type: 'other' }));
    await page.waitForTimeout(200);
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'goal_c', type: 'other' }));
    await page.waitForTimeout(500);
  ...
  test('queues payloads when offline and sends when back online', async ({
    ...
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'offline_goal', type: 'other' }));
    await page.waitForTimeout(500);
  ...
  test('handles 500 server error gracefully', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'error_test', type: 'other' }));
    await page.waitForTimeout(500);
  ...
  test('handles 429 rate limit gracefully', async ({ page }) => {
    ...
    // Track goal - should not crash
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'rate_limit_test', type: 'other' }));
    await page.waitForTimeout(500);
  ...
  test.skip('SDK remains functional when requests timeout', async ({ page }) => {
    ...
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'timeout_test', type: 'other' }));
    ...
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'after_timeout', type: 'other' }));
