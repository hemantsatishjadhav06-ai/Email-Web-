...
test.describe('Data Persistence', () => {
  ...
  test('custom dimensions persist in localStorage', async ({ page }) => {
    ...
    await page.evaluate(async () => {
      await MailwaveAnalytics.setDimension(1, 'value1');
      await MailwaveAnalytics.setDimension(2, 'value2');
      await MailwaveAnalytics.setDimension(5, 'value5');
    });
    ...
    const dim1 = await page.evaluate(() => MailwaveAnalytics.getDimension(1));
    const dim2 = await page.evaluate(() => MailwaveAnalytics.getDimension(2));
    const dim5 = await page.evaluate(() => MailwaveAnalytics.getDimension(5));
  ...
  test('session state persists in sessionStorage', async ({ page }) => {
    ...
    // Track a goal
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'persist_test', type: 'other' }));
  ...
  test('session state restored after navigation', async ({ page }) => {
    ...
    // Track a goal
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'before_nav', type: 'other' }));
    await page.waitForTimeout(500);
    ...
    // Track another goal
    await page.evaluate(() => MailwaveAnalytics.trackGoal({ action: 'after_nav', type: 'other' }));
    await page.waitForTimeout(500);
  ...
  test('dimensions persist across page reload', async ({ page }) => {
    ...
    // Set dimensions
    await page.evaluate(async () => {
      await MailwaveAnalytics.setDimension(1, 'premium');
      await MailwaveAnalytics.setDimension(2, 'enterprise');
    });
  ...
  test('clearDimensions removes all custom dimensions', async ({ page }) => {
    ...
    // Verify they exist
    const dimBefore = await page.evaluate(() => MailwaveAnalytics.getDimension(1));
    expect(dimBefore).toBe('value1');

    // Clear all dimensions
    await page.evaluate(() => MailwaveAnalytics.clearDimensions());
