...
test.describe('Comprehensive Session Verification', () => {
  ...
  test('multi-page session with full SDK API coverage and database verification', async ({ page }) => {
    ...
    const config = await page.evaluate(() => MailwaveAnalytics.getConfig());
    expect(config).toBeTruthy();
    expect(config?.workspace_id).toBe('test_workspace');

    const debugInfo = await page.evaluate(() => MailwaveAnalytics.debug());
    ...
    // ========================================
    await page.evaluate(() => {
      MailwaveAnalytics.setDimension(1, 'dimension_1_value');
      MailwaveAnalytics.setDimension(2, 'dimension_2_value');
    });
    ...
    // ========================================
    await page.evaluate(() => MailwaveAnalytics.pause());
    await page.waitForTimeout(1000); // 1 second paused (should NOT count)
    await page.evaluate(() => MailwaveAnalytics.resume());
    ...
    // ========================================
    await page.evaluate(() => MailwaveAnalytics.trackPageView('/custom-path'));
    await page.waitForTimeout(500);

    // ========================================
    // Verify Duration APIs
    // ========================================
    const focusDuration = await page.evaluate(() => MailwaveAnalytics.getFocusDuration());
    const totalDuration = await page.evaluate(() => MailwaveAnalytics.getTotalDuration());
    ...
    for (const event of events) {
      ...
      // Mailwave releases are vMAJOR.minor (config/config.go), not semver
