# MailwaveAnalytics Web SDK v5.0
...
Your Mailwave server serves the SDK at `/na.js`, built from the same release
...
window.MailwaveAnalyticsConfig = {
  ...
  endpoint: 'https://your-mailwave-instance.com',
  debug: false // optional
};
</script>
<script async src="https://your-mailwave-instance.com/na.js"></script>
```

The SDK auto-initializes from `window.MailwaveAnalyticsConfig`. No explicit
...
```typescript
// Session info (async)
await MailwaveAnalytics.getSessionId();       // Current session UUID
await MailwaveAnalytics.getFocusDuration();   // Active time in milliseconds
await MailwaveAnalytics.getTotalDuration();   // Wall clock time in milliseconds

// Synchronous methods
MailwaveAnalytics.getConfig();                // Returns config or null
MailwaveAnalytics.debug();                    // Get debug info

// Manual tracking (async)
await MailwaveAnalytics.trackPageView(url?);  // Track SPA navigation
await MailwaveAnalytics.trackGoal({ action, type, value?, properties? });
...
await MailwaveAnalytics.setDimension(1, 'premium');    // Set custom_1 = 'premium'
await MailwaveAnalytics.setDimensions({ 1: 'a', 2: 'b' }); // Set multiple
await MailwaveAnalytics.getDimension(1);               // Get dimension value
await MailwaveAnalytics.clearDimensions();             // Clear all

// Control (async)
await MailwaveAnalytics.pause();              // Pause tracking
await MailwaveAnalytics.resume();             // Resume tracking
await MailwaveAnalytics.reset();              // Clear session, start fresh
```

## Configuration

Set `window.MailwaveAnalyticsConfig` before loading the SDK script:

```typescript
interface MailwaveAnalyticsConfig {
  // Required
...
declare global {
  interface Window {
    MailwaveAnalyticsConfig?: MailwaveAnalyticsConfig;
  }
}
```
