...
describe('WebhookCard', () => {
  ...
  it('reports the reason and failure count when Mailwave disabled the subscription', async () => {
    await renderCard({
      enabled: false,
    ...
    expect(screen.getByText('Mailwave disabled this webhook automatically')).toBeInTheDocument()
  ...
  it('claims no reason when the user disabled the subscription', async () => {
    ...
    expect(screen.queryByText(/Mailwave disabled this webhook/)).toBeNull()
