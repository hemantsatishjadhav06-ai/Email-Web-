...
const EmailIntegration = ({
  ...
  // SES receipt rule + SNS topic) that forwards replies to Mailwave (for automation
  ...
  const renderInboundReplyStatus = () => {
    ...
    return (
      <div className="mb-2">
        <Tooltip
          title={t`Forwards inbound replies to Mailwave so automations can stop when a contact replies (Exit on reply). Registering webhooks creates the provider-side route; you must also point your domain's MX records at your email provider.`}
        >
    ...
    if (!alreadyProvisioned) {
      const confirmed = await new Promise<boolean>((resolve) => {
        Modal.confirm({
          ...
          content: t`Mailwave will create a tenant in your AWS account, give it its own suppression list, and associate this integration's configuration set and sender identities with it. AWS bills per tenant per month, based on volume.`,
        ...
        {providerType === 'ses' && (
          <>
            ...
            <Form.Item
              ...
              extra={t`Leave empty to use the one Mailwave manages for this integration.`}
              rules={[
                {
            ...
            <Form.Item
              ...
              rules={[
                ...
                // The server refuses both at once (AmazonSESSettings.Validate): a Mailwave-managed
