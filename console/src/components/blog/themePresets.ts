      ...
      <div class="footer-content">
        <div class="footer-left">
          ...
          <p class="text-gray-500 text-xs mt-1">
            Powered by <a href="https://mailwave-production-72a7.up.railway.app" target="_blank" rel="noopener" style="color: var(--color-link); text-decoration: none;">Mailwave</a>
          </p>
        </div>
        ...
        <div class="footer-links">
          ...
          <div class="social-links">
            <a href="https://mailwave-production-72a7.up.railway.app" target="_blank" rel="noopener noreferrer" aria-label="Website">
              <svg class="social-icon" fill="currentColor" viewBox="0 0 24 24">
                <path
                  d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z" />
              </svg>
            </a>
            <a href="https://x.com/mailwave" target="_blank" rel="noopener noreferrer" aria-label="X (Twitter)">
              <svg class="social-icon" fill="currentColor" viewBox="0 0 24 24">
                <path
                  d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
            </a>
            <a href="https://github.com/Mailwave/mailwave" target="_blank" rel="noopener noreferrer"
              aria-label="GitHub">
              <svg class="social-icon" fill="currentColor" viewBox="0 0 24 24">
...
/**
 * Subscribe a contact to newsletter lists via Mailwave API
...
async function subscribeToNewsletter(email, firstName = null) {
  try {
    const response = await fetch(\`\${MAILWAVE_CONFIG.domain}/subscribe\`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        workspace_id: MAILWAVE_CONFIG.workspaceId,
        contact: {
          email: email,
          first_name: firstName || null
        },
        list_ids: MAILWAVE_CONFIG.listIds
      })
    });
...
function initNewsletterForms() {
  ...
  forms.forEach(form => {
    form.addEventListener('submit', async function (e) {
      ...
      if (!MAILWAVE_CONFIG.listIds || MAILWAVE_CONFIG.listIds.length === 0) {
