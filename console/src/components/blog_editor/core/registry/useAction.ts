...
import { useMailwaveEditor } from '../../hooks/useEditor'
import { mailwaveActionRegistry } from './ActionRegistry'
...
export function useAction(actionId: string, config?: UseActionConfig): ActionState {
  ...
  const { editor } = useMailwaveEditor(providedEditor)

  // Get the action definition from the registry
  const action = mailwaveActionRegistry.get(actionId)
