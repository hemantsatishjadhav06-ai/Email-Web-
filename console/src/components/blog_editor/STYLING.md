# Mailwave Editor - Dynamic Styling Guide

This guide explains how to customize the Mailwave Editor's appearance through the `styleConfig` prop.

## Overview

The Mailwave Editor supports dynamic styling through a type-safe JSON configuration. This allows you to customize fonts, colors, sizes, and spacing for all text elements in the editor.

## Table of Contents

- [Quick Start](#quick-start)
- [Style Configuration Reference](#style-configuration-reference)
- [Integration with Mailwave Go Backend](#integration-with-mailwave-go-backend)
...
function MyEditor() {
  return <MailwaveEditor />
}

// Or explicitly pass default styles
function MyEditorWithDefaults() {
  return <MailwaveEditor styleConfig={defaultEditorStyles} />
}
...
function MyCustomEditor() {
  return <MailwaveEditor styleConfig={customStyles} />
}
```
...
## Integration with Mailwave Go Backend
...
function BlogEditorComponent({ configFromBackend }: EditorComponentProps) {
  ...
  return <MailwaveEditor styleConfig={validConfig} />
}
```
...
```typescript
import { MailwaveEditor, timesJournalPreset } from '@/components/blog_editor'

;<MailwaveEditor styleConfig={timesJournalPreset} />
```
...
```typescript
import { MailwaveEditor, modernMagazinePreset } from '@/components/blog_editor'

;<MailwaveEditor styleConfig={modernMagazinePreset} />
```
...
```typescript
import { MailwaveEditor, minimalBlogPreset } from '@/components/blog_editor'

;<MailwaveEditor styleConfig={minimalBlogPreset} />
```
...
```typescript
import { MailwaveEditor, academicPaperPreset } from '@/components/blog_editor'

;<MailwaveEditor styleConfig={academicPaperPreset} />
```
...
For issues or questions about dynamic styling, refer to the Mailwave Editor documentation or contact support.
