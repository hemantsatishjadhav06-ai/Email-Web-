...
export interface WebAnalyticsSettings {
  ...
  /** Off by default; letting Mailwave identify recipients from tracked email links is opt-in. */
...
export function buildInstallSnippet(endpoint: string, workspaceId: string): string {
  ...
  return [
    '<script>',
    `  window.MailwaveAnalyticsConfig = { workspace_id: ${JSON.stringify(workspaceId)}, endpoint: ${JSON.stringify(origin)} };`,
    '</script>',
