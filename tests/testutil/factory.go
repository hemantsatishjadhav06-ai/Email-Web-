...
func (tdf *TestDataFactory) CreateMailpitSMTPIntegration(workspaceID string, opts ...IntegrationOption) (*domain.Integration, error) {
	mailpitOpts := []IntegrationOption{
		...
		WithIntegrationEmailProvider(domain.EmailProvider{
			Kind: domain.EmailProviderKindSMTP,
			Senders: []domain.EmailSender{
				domain.NewEmailSender("noreply@mailwave.test", "Mailwave Test"),
			},
			SMTP: &domain.SMTPSettings{
...
func createDefaultMJMLBlock() mailwave_mjml.EmailBlock {
	...
	block, err := mailwave_mjml.UnmarshalEmailBlock(jsonData)
	if err != nil {
		panic(err)
	}
...
func CreateMJMLBlockWithContent(content string) mailwave_mjml.EmailBlock {
	textBlockMap := map[string]interface{}{
		"id":      "text-1",
		"type":    "mj-text",
		"content": content,
...
func CreateMJMLBlockWithContentAndPreview(content string, previewContent string) mailwave_mjml.EmailBlock {
