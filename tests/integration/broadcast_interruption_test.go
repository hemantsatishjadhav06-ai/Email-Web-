...
func slowEnqueue(t *testing.T, h *phase2Harness, d time.Duration) func() {
	...
	_, err = wsDB.Exec(fmt.Sprintf(`CREATE OR REPLACE FUNCTION mailwave_test_slow_enqueue() RETURNS trigger AS $$
		BEGIN PERFORM pg_sleep(%f); RETURN NULL; END;
		$$ LANGUAGE plpgsql`, d.Seconds()))
	require.NoError(t, err)
	_, err = wsDB.Exec(`CREATE TRIGGER mailwave_test_slow_enqueue
		BEFORE INSERT ON email_queue
		FOR EACH STATEMENT EXECUTE FUNCTION mailwave_test_slow_enqueue()`)
	require.NoError(t, err)

	return func() {
		_, _ = wsDB.Exec(`DROP TRIGGER IF EXISTS mailwave_test_slow_enqueue ON email_queue`)
	}
}
...
func TestBroadcastEnqueueFailure_DoesNotSkipRecipients(t *testing.T) {
	...
	_, err = wsDB.Exec(`CREATE OR REPLACE FUNCTION mailwave_test_fail_enqueue() RETURNS trigger AS $$
		BEGIN RAISE EXCEPTION 'injected enqueue failure'; END;
		$$ LANGUAGE plpgsql`)
	require.NoError(t, err)
	_, err = wsDB.Exec(`CREATE TRIGGER mailwave_test_fail_enqueue
		BEFORE INSERT ON email_queue
		FOR EACH STATEMENT EXECUTE FUNCTION mailwave_test_fail_enqueue()`)
	require.NoError(t, err)
	...
	clearFault := func() {
		...
		_, dropErr := wsDB.Exec(`DROP TRIGGER IF EXISTS mailwave_test_fail_enqueue ON email_queue`)
...
func TestBroadcastRetryBudget_IsRepaidByProgress(t *testing.T) {
	...
	_, err = wsDB.Exec(`DROP TRIGGER IF EXISTS mailwave_test_fail_enqueue ON email_queue`)
	require.NoError(t, err)
