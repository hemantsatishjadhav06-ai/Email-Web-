...
func TestBroadcastLiquidTemplateSubstitution(t *testing.T) {
	...
	_, err = factory.SetupWorkspaceWithSMTPProvider(workspace.ID,
		testutil.WithIntegrationEmailProvider(domain.EmailProvider{
			...
			Senders: []domain.EmailSender{
				domain.NewEmailSender("noreply@mailwave.test", "Mailwave Liquid Test"),
			},
	...
	t.Run("renders workspace.website_url and workspace.base_url for composing app links", func(t *testing.T) {
		...
		err = factory.SetWorkspaceWebsiteURL(workspace.ID, "https://app.mailwave-verify.test/")
		require.NoError(t, err)
		...
		err = factory.SetWorkspaceCustomEndpointURL(workspace.ID, "https://track.mailwave-verify.test")
		require.NoError(t, err)
		...
		expectedVerifyURL := "https://app.mailwave-verify.test" + verifyPath
		...
		assert.NotContains(t, msg.HTML, "https://app.mailwave-verify.test//",
			"trailing slash should be trimmed so no double slash appears")
		// website_url is exposed standalone (trailing slash trimmed)
		assert.Contains(t, msg.HTML, "site=[https://app.mailwave-verify.test]",
		...
		assert.Contains(t, msg.HTML, "base=[https://track.mailwave-verify.test]",
