...
import (
	...
	"github.com/Mailwave/mailwave/pkg/templates"
...
func TestTemplateRoundtripSerialization(t *testing.T) {
	...
	t.Run("Roundtrip with all component types", func(t *testing.T) {
		...
		assert.Equal(t, mailwave_mjml.MJMLComponentMjml, result.Template.Email.VisualEditorTree.GetType())
	})
	...
	t.Run("Production Supabase templates can be round-tripped", func(t *testing.T) {
		...
		for name, createFunc := range supabaseTemplates {
			t.Run(name, func(t *testing.T) {
				...
				assert.Equal(t, mailwave_mjml.MJMLComponentMjml,
...
func createAllComponentsTree(t *testing.T) mailwave_mjml.EmailBlock {
	jsonTemplate := `{
		"id": "mjml-root",
		"type": "mjml",
		"attributes": {},
	...
	block, err := mailwave_mjml.UnmarshalEmailBlock([]byte(jsonTemplate))
...
func createNestedStructure(t *testing.T) mailwave_mjml.EmailBlock {
	jsonTemplate := `{
		"id": "mjml-1",
		"type": "mjml",
		"attributes": {},
