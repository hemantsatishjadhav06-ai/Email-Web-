...
func TestWorkspaceRepository_DeleteDatabase(t *testing.T) {
	...
	const dbName = "mailwave_ws_testworkspace"
	...
	t.Run("drops the database with FORCE", func(t *testing.T) {
		repo, mock, cleanup := newRepo(t, "mailwave")
		defer cleanup()
	...
	t.Run("drops the database even if closing the pool fails", func(t *testing.T) {
		...
		repo := NewWorkspaceRepository(db,
			&config.DatabaseConfig{User: "postgres", DBName: "mailwave_system", Prefix: "mailwave"},
	...
	t.Run("lower-cases the database name to match unquoted creation", func(t *testing.T) {
		...
		lowered := fmt.Sprintf(`DROP DATABASE IF EXISTS %s WITH (FORCE)`, pq.QuoteIdentifier("mailwave_ws_testworkspace"))
	...
	t.Run("replaces hyphens in the workspace ID", func(t *testing.T) {
		...
		hyphenated := fmt.Sprintf(`DROP DATABASE IF EXISTS %s WITH (FORCE)`, pq.QuoteIdentifier("mailwave_ws_test_ws_1"))
