...
func TestWorkspaceRepository_GetByID_Postgres(t *testing.T) {
	...
	dbConfig := &config.DatabaseConfig{Prefix: "mailwave"}
...
func TestWorkspaceRepository_checkWorkspaceIDExists_Postgres(t *testing.T) {
	...
	connMgr := newMockConnectionManager(db)
	repo := NewWorkspaceRepository(db, &config.DatabaseConfig{Prefix: "mailwave"}, "secret-key", connMgr).(*workspaceRepository)
...
func TestWorkspaceRepository_Delete_Postgres(t *testing.T) {
	...
	dbConfig := &config.DatabaseConfig{User: "postgres", Prefix: "mailwave"}
	connMgr := newMockConnectionManager(db)
...
func TestWorkspaceRepository_WithWorkspaceTransaction(t *testing.T) {
	...
	repo := NewWorkspaceRepository(systemDB, &config.DatabaseConfig{Prefix: "mailwave"}, "secret-key", connMgr).(*workspaceRepository)
	workspaceID := "ws_tx"
