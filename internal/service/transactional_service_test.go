			...
			mockSetup: func() {
				...
				mockRepo.EXPECT().
					...
					DoAndReturn(func(_ context.Context, _ string, notif *domain.TransactionalNotification) error {
						...
						assert.Equal(t, mailwave_mjml.TrackingSettings{
							TrackingMode: mailwave_mjml.TrackingModeDisabled,
							UTMSource:    "stored-source",
		...
		},
		{
			...
			mockSetup: func() {
				...
				mockRepo.EXPECT().
					Update(gomock.Any(), workspace, gomock.Any()).
					DoAndReturn(func(_ context.Context, _ string, notif *domain.TransactionalNotification) error {
						assert.Equal(t, mailwave_mjml.TrackingModeDisabled, notif.TrackingSettings.TrackingMode)
	...
	t.Run("Success_DisabledModeSuppressesWorkspaceTracking", func(t *testing.T) {
		...
		mockEmailService.EXPECT().
			...
			Do(func(_ context.Context, request domain.SendEmailRequest) {
				...
				assert.Equal(t, mailwave_mjml.TrackingModeDisabled, request.TrackingSettings.TrackingMode)
			}).Return(nil)
		...
		Translations: map[string]domain.TemplateTranslation{
			"fr": {
				Email: &domain.EmailTemplate{
					...
					VisualEditorTree: &mailwave_mjml.MJMLBlock{
						BaseBlock: mailwave_mjml.NewBaseBlock("root-fr", mailwave_mjml.MJMLComponentMjml),
					},
				},
			},
		},
...
func TestEffectiveTracking(t *testing.T) {
	tests := []struct {
		...
		{"workspace on, explicit inherit", true, mailwave_mjml.TrackingModeInherit, true},
		{"workspace on, disabled wins", true, mailwave_mjml.TrackingModeDisabled, false},
		{"workspace off is a kill-switch for unset", false, "", false},
		{"workspace off is a kill-switch for inherit", false, mailwave_mjml.TrackingModeInherit, false},
		{"workspace off, disabled", false, mailwave_mjml.TrackingModeDisabled, false},
...
func TestCanonicalTrackingMode(t *testing.T) {
	...
	assert.Equal(t, "", canonicalTrackingMode(mailwave_mjml.TrackingModeInherit))
	assert.Equal(t, mailwave_mjml.TrackingModeDisabled, canonicalTrackingMode(mailwave_mjml.TrackingModeDisabled))
}
...
func TestTransactionalNotificationService_UpdateNotification_TrackingSettingsPresence(t *testing.T) {
	...
	tests := []struct {
		name     string
		body     string
		expected mailwave_mjml.TrackingSettings
	}{
		{
