...
func TestDemoService_CompileTemplateToHTML_Basic(t *testing.T) {
	...
	head := &mailwave_mjml.MJHeadBlock{BaseBlock: mailwave_mjml.NewBaseBlock("head", mailwave_mjml.MJMLComponentMjHead)}
	head.Children = []mailwave_mjml.EmailBlock{title}

	textBase := mailwave_mjml.NewBaseBlock("text", mailwave_mjml.MJMLComponentMjText)
	textBase.Content = &textContent
	text := &mailwave_mjml.MJTextBlock{BaseBlock: textBase}

	col := &mailwave_mjml.MJColumnBlock{BaseBlock: mailwave_mjml.NewBaseBlock("col", mailwave_mjml.MJMLComponentMjColumn)}
	col.Children = []mailwave_mjml.EmailBlock{text}

	sec := &mailwave_mjml.MJSectionBlock{BaseBlock: mailwave_mjml.NewBaseBlock("sec", mailwave_mjml.MJMLComponentMjSection)}
	sec.Children = []mailwave_mjml.EmailBlock{col}

	body := &mailwave_mjml.MJBodyBlock{BaseBlock: mailwave_mjml.NewBaseBlock("body", mailwave_mjml.MJMLComponentMjBody)}
	body.Children = []mailwave_mjml.EmailBlock{sec}

	root := &mailwave_mjml.MJMLBlock{BaseBlock: mailwave_mjml.NewBaseBlock("root", mailwave_mjml.MJMLComponentMjml)}
	root.Children = []mailwave_mjml.EmailBlock{head, body}
...
func TestDemoService_CreateNewsletterStructures_NotNil(t *testing.T) {
	...
	assert.NotNil(t, b3)
	assert.NotNil(t, b4)
	assert.Equal(t, mailwave_mjml.MJMLComponentMjml, b1.GetType())
	assert.Equal(t, mailwave_mjml.MJMLComponentMjml, b2.GetType())
	assert.Equal(t, mailwave_mjml.MJMLComponentMjml, b3.GetType())
	assert.Equal(t, mailwave_mjml.MJMLComponentMjml, b4.GetType())
}
...
func TestDemoService_CompileTemplateToHTML_WithMinimalInput(t *testing.T) {
	...
	minimalBlock := &mailwave_mjml.MJTextBlock{BaseBlock: mailwave_mjml.NewBaseBlock("minimal", mailwave_mjml.MJMLComponentMjText)}
...
func TestDemoService_CompileTemplateToHTML_CompilationFailure(t *testing.T) {
	...
	invalidText := &mailwave_mjml.MJTextBlock{BaseBlock: mailwave_mjml.NewBaseBlock("invalid", mailwave_mjml.MJMLComponentMjText)}
	invalidText.Content = nil // Invalid content

	// Create a minimal but potentially problematic structure
	body := &mailwave_mjml.MJBodyBlock{BaseBlock: mailwave_mjml.NewBaseBlock("body", mailwave_mjml.MJMLComponentMjBody)}
	body.Children = []mailwave_mjml.EmailBlock{invalidText}

	root := &mailwave_mjml.MJMLBlock{BaseBlock: mailwave_mjml.NewBaseBlock("root", mailwave_mjml.MJMLComponentMjml)}
	root.Children = []mailwave_mjml.EmailBlock{body}
