...
func (s *EmailService) TestEmailProvider(ctx context.Context, workspaceID string, integrationID string, provider domain.EmailProvider, to string) error {
	...
	subject := "Mailwave: Test Email Provider"
	content := "<h1>Mailwave: Test Email Provider</h1><p>This is a test email from Mailwave. Your provider is working!</p>"
...
func sanitizeClickedURL(clickedURL string, requestHost string) string {
	...
	// The parameter name has to be the one mailwave_mjml WRITES onto a tracked
...
func (s *EmailService) SendEmailForTemplate(ctx context.Context, request domain.SendEmailRequest) error {
	...
	trackingOptedOut := request.TrackingSettings.TrackingMode == mailwave_mjml.TrackingModeDisabled
	...
	subject, err := mailwave_mjml.ProcessLiquidTemplate(
	...
	if request.EmailOptions.Subject != nil && *request.EmailOptions.Subject != "" {
		overrideSubject, err := mailwave_mjml.ProcessLiquidTemplate(
