...
//go:generate mockgen -destination=./mocks/mock_message_sender.go -package=mocks github.com/Mailwave/mailwave/internal/service/broadcast MessageSender
...
// The link rewriter reads the list the opposite way — mailwave_mjml.
...
func (s *messageSender) SendToRecipient(ctx context.Context, workspaceID string, integrationID string, workspaceSecretKey string, endpoint string, trackingEnabled bool, webAnalytics *domain.WebAnalyticsSettings, broadcast *domain.Broadcast, messageID string, email string,
	...
	compiledTemplate, err := mailwave_mjml.CompileTemplate(compileReq)
	...
	processedSubject, err := mailwave_mjml.ProcessLiquidTemplate(
