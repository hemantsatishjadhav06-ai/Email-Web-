...
import (
	...
	bmocks "github.com/Mailwave/mailwave/internal/service/broadcast/mocks"
...
func TestSendToRecipientCompileFailure(t *testing.T) {
	...
	template := &domain.Template{
		...
		Email: &domain.EmailTemplate{
			SenderID:         emailSender.ID,
			Subject:          "Test Subject",
			VisualEditorTree: &mailwave_mjml.MJMLBlock{}, // Empty block should cause compilation issues
		},
	}
...
func TestSendToRecipient_CompilationFailsWithNilHTML(t *testing.T) {
	...
	template := &domain.Template{
		...
		Email: &domain.EmailTemplate{
			SenderID:         emailSender.ID,
			Subject:          "Test Subject",
			VisualEditorTree: &mailwave_mjml.MJMLBlock{}, // Minimal block
		},
	}
...
func TestMintIdentifyToken(t *testing.T) {
	t.Run("closed gates mint nothing", func(t *testing.T) {
		closed := []struct {
			...
			// ours: everything else is configured, but nobody asked Mailwave to
