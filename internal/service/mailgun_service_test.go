...
func newMailgunTestService(ctrl *gomock.Controller) (*MailgunService, *mocks.MockHTTPClient) {
	...
	svc := NewMailgunService(httpClient, mocks.NewMockAuthService(ctrl), logger, "https://api.mailwave.test/webhooks/email")
...
func TestMailgunService_RegisterWebhooks_Coexistence(t *testing.T) {
	const (
		...
		baseURL       = "https://api.mailwave.test"
	...
	t.Run("stale own URL is replaced, co-tenant preserved", func(t *testing.T) {
		...
		staleSelf := domain.GenerateWebhookCallbackURL("https://old.mailwave.test", domain.EmailProviderKindMailgun, workspaceID, integrationID)
...
func TestMailgunService_EnsureInboundRoute(t *testing.T) {
	ctx := context.Background()
	const inboundURL = "https://api.mailwave.test/webhooks/email/inbound?workspace_id=ws1&integration_id=int1"
	newProvider := func() *domain.EmailProvider {
