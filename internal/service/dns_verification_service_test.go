...
func TestDNSVerificationService_VerifyTXTRecord_ExpectedRecordFormat(t *testing.T) {
	...
	expectedRecord := "mailwave-verify=" + token

	// Verify the format matches what the code expects
	assert.Equal(t, "mailwave-verify=test-verification-token-123", expectedRecord)
	...
	for _, token := range tokens {
		t.Run("Token format: "+token, func(t *testing.T) {
			expected := "mailwave-verify=" + token
			assert.Equal(t, expected, "mailwave-verify="+token)
		})
	}
}
