...
func (s *TemplateService) stampEmailMetadata(email *domain.EmailTemplate, title string) {
	...
	s.updateBlockContentRecursively(email.VisualEditorTree, mailwave_mjml.MJMLComponentMjTitle, title)
	s.updateBlockContentRecursively(email.VisualEditorTree, mailwave_mjml.MJMLComponentMjPreview, previewText)
}

// updateBlockContentRecursively traverses the email block tree and updates content for blocks of the specified type
func (s *TemplateService) updateBlockContentRecursively(block mailwave_mjml.EmailBlock, blockType mailwave_mjml.MJMLComponentType, content string) {
	if block == nil {
		return
	}
	...
	if block.GetType() == blockType {
		...
		case *mailwave_mjml.MJTitleBlock:
			typedBlock.Content = &content
		case *mailwave_mjml.MJPreviewBlock:
			typedBlock.Content = &content
		}
	}
	...
	if needBaseURL || needWebsiteURL {
		...
		if existingWorkspace != nil {
			...
			if payload.TemplateData == nil {
				payload.TemplateData = mailwave_mjml.MapOfAny{}
			}
			payload.TemplateData["workspace"] = vars
		}
	}

	resp, err := mailwave_mjml.CompileTemplate(payload)
