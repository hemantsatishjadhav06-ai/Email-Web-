...
func effectiveTracking(workspaceEnabled bool, mode string) bool {
	return workspaceEnabled && mode != mailwave_mjml.TrackingModeDisabled
}
...
func canonicalTrackingMode(mode string) string {
	if mode == mailwave_mjml.TrackingModeInherit {
		return ""
	}
	return mode
}
...
func (s *TransactionalNotificationService) UpdateNotification(
	...
	if params.TrackingSettingsSpecified() {
		...
		case mailwave_mjml.TrackingModeInherit:
...
func (s *TransactionalNotificationService) TestTemplate(ctx context.Context, workspaceID string, templateID string, integrationID string, senderID string, recipientEmail string, language string, emailOptions domain.EmailOptions) error {
	...
	compileReq := domain.CompileTemplateRequest{
		WorkspaceID:      workspaceID,
		MessageID:        messageID,
		TemplateData:     mailwave_mjml.MapOfAny(messageData),
		TrackingSettings: trackingSettings,
	}
