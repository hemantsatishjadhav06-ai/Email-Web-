...
func TestEmailService_VisitLink(t *testing.T) {
	...
	emailService := EmailService{
		...
		apiEndpoint:     "https://api.mailwave.test",
	}

	ctx := context.Background()
	workspaceID := "workspace-123"
	messageID := "message-456"
	requestHost := "click.mailwave.test"
	...
	t.Run("URL pointing back at the request host degrades to aggregate-only", func(t *testing.T) {
		...
		err := emailService.VisitLink(ctx, messageID, workspaceID,
			"https://click.mailwave.test/unsubscribe?email=someone@example.com", requestHost)
		require.NoError(t, err)
	})
	...
	t.Run("Host comparison is case-insensitive", func(t *testing.T) {
		...
		err := emailService.VisitLink(ctx, messageID, workspaceID,
			"https://Click.Mailwave.Test/notification-center?email=someone@example.com", requestHost)
		require.NoError(t, err)
	})
...
func TestSanitizeClickedURL_CollapsesPerRecipientIdentityTokens(t *testing.T) {
	...
	const requestHost = "click.mailwave.test"
...
func TestSanitizeClickedURL_IdentifyTokenStripping(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			url:      "https://click.mailwave.test/unsubscribe?email=someone@example.com&nf_id=abc123",
			expected: "",
		},
		{
...
func TestEmailService_SendEmailForTemplate(t *testing.T) {
	...
	emailTemplate := &domain.Template{
		...
		Email: &domain.EmailTemplate{
			...
			VisualEditorTree: &mailwave_mjml.MJMLBlock{
				BaseBlock: mailwave_mjml.NewBaseBlock("root", mailwave_mjml.MJMLComponentMjml),
			},
		},
	}
	...
	t.Run("TrackingMode survives the compile-request rebuild", func(t *testing.T) {
		...
		mockTemplateService.EXPECT().
			...
			DoAndReturn(func(_ context.Context, compileReq domain.CompileTemplateRequest) (*domain.CompileTemplateResponse, error) {
				assert.Equal(t, mailwave_mjml.TrackingModeDisabled, compileReq.TrackingSettings.TrackingMode,
		...
		request := domain.SendEmailRequest{
			...
			TrackingSettings: mailwave_mjml.TrackingSettings{
				...
				TrackingMode:   mailwave_mjml.TrackingModeDisabled,
			},
			EmailProvider: emailProvider,
			EmailOptions:  options,
		}
...
const capturedTrackingEndpoint = "https://track.mailwave.test"
...
func sendForTemplateCapturingTracking(t *testing.T, webAnalytics *domain.WebAnalyticsSettings, contactEmail string, secretKey string) mailwave_mjml.TrackingSettings {
	t.Helper()
...
func sendForTemplateCapturingTrackingWithOptions(t *testing.T, webAnalytics *domain.WebAnalyticsSettings, contactEmail string, secretKey string, emailOptions domain.EmailOptions) mailwave_mjml.TrackingSettings {
	t.Helper()
	return sendForTemplateCapturingTrackingWithRequestSettings(t, webAnalytics, contactEmail, secretKey, emailOptions,
		mailwave_mjml.TrackingSettings{EnableTracking: true})
}
...
func sendForTemplateCapturingTrackingWithRequestSettings(t *testing.T, webAnalytics *domain.WebAnalyticsSettings, contactEmail string, secretKey string, emailOptions domain.EmailOptions, requestTracking mailwave_mjml.TrackingSettings) mailwave_mjml.TrackingSettings {
	t.Helper()
	...
	var captured mailwave_mjml.TrackingSettings
	mockTemplateService.EXPECT().
		CompileTemplate(gomock.Any(), gomock.Any()).
...
func TestEmailService_SendEmailForTemplate_WebIdentityTrackingMode(t *testing.T) {
	...
	send := func(t *testing.T, requestTracking mailwave_mjml.TrackingSettings) mailwave_mjml.TrackingSettings {
		t.Helper()
	...
	t.Run("Mints nothing when the notification opts out of tracking", func(t *testing.T) {
		...
		tracking := send(t, mailwave_mjml.TrackingSettings{
			TrackingMode:   mailwave_mjml.TrackingModeDisabled,
			EnableTracking: false,
		})
	...
	t.Run("Mints on an explicit inherit", func(t *testing.T) {
		tracking := send(t, mailwave_mjml.TrackingSettings{
			TrackingMode:   mailwave_mjml.TrackingModeInherit,
			EnableTracking: true,
		})
	...
	t.Run("Mints on the absent mode", func(t *testing.T) {
		...
		tracking := send(t, mailwave_mjml.TrackingSettings{EnableTracking: true})
	...
	t.Run("Mints when click tracking is off but the notification did not opt out", func(t *testing.T) {
		...
		for _, mode := range []string{"", mailwave_mjml.TrackingModeInherit} {
			tracking := send(t, mailwave_mjml.TrackingSettings{
				TrackingMode:   mode,
				EnableTracking: false,
			})
...
// self-consistent. The name that matters is the one mailwave_mjml WRITES onto a
...
func TestSanitizeClickedURL_StripsWhatTrackLinksWrites(t *testing.T) {
	const secretKey = "workspace-secret-key-32-chars-min!"
	const requestHost = "track.mailwave.test"
	...
	destinationFor := func(recipient string) string {
		...
		trackedHTML, err := mailwave_mjml.TrackLinks(linkHTML, tracking)
		require.NoError(t, err)
