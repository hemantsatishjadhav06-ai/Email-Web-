...
func TestSESService_SendEmail_SetsTenantName(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			mutate: func(s *domain.AmazonSESSettings) {
				...
				s.ManagedTenantName = "mailwave-int-1"
			},
			expected: aws.String("mailwave-int-1"),
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			settings.ManagedConfigurationSet = "mailwave-int-1" // avoids any lookup
			tc.mutate(settings)
...
func TestSESService_SendEmail_UsesConfiguredConfigurationSet(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			settings: func() *domain.AmazonSESSettings {
				s := sesSettings()
				s.ManagedConfigurationSet = "mailwave-int-1"
				return s
			},
			expected: "mailwave-int-1",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
	...
	t.Run("positive result is looked up once", func(t *testing.T) {
		...
		mockSES.EXPECT().ListConfigurationSetsWithContext(gomock.Any(), gomock.Any()).
			Return(&ses.ListConfigurationSetsOutput{
				ConfigurationSets: []*ses.ConfigurationSet{{Name: aws.String("mailwave-int-1")}},
			}, nil).
			Times(1)

		for i := 0; i < 3; i++ {
			got := service.resolveConfigurationSet(context.Background(), *sesSettings(), "int-1")
			assert.Equal(t, "mailwave-int-1", got)
		}
	})
...
func TestSESService_SendEmail_RawPathDeliversCCWithBCC(t *testing.T) {
	...
	settings := sesSettings()
	settings.ManagedConfigurationSet = "mailwave-int-1"
...
func TestSESService_SendEmail_BuildsExpectedInput(t *testing.T) {
	...
	settings := sesSettings()
	settings.ManagedConfigurationSet = "mailwave-int-1"
	settings.ManagedTenantName = "mailwave-int-1"
	...
	assert.Equal(t, "mailwave-int-1", aws.StringValue(captured.ConfigurationSetName))
	assert.Equal(t, "mailwave-int-1", aws.StringValue(captured.TenantName))
	...
	assert.Equal(t, "mailwave_message_id", aws.StringValue(captured.EmailTags[0].Name))
...
func TestSESService_UnregisterWebhooks_KeepsConfigurationSetWhenTenantInPlay(t *testing.T) {
	for _, tc := range []struct {
		...
		{"managed tenant", func(s *domain.AmazonSESSettings) { s.ManagedTenantName = "mailwave-int-1" }},
...
func TestConfigurationSetFor(t *testing.T) {
	...
	assert.Equal(t, "mailwave-int-1", managedName)
...
func TestPreserveDerivedSESFields(t *testing.T) {
	existing := &domain.Integration{
		...
		EmailProvider: domain.EmailProvider{
			...
			SES: &domain.AmazonSESSettings{
				ManagedConfigurationSet: "mailwave-int-1",
				ManagedTenantName:       "mailwave-int-1",
				InboundTopicARN:         "arn:aws:sns:eu-west-3:123456789012:mailwave-ses-int-1",
			},
		},
	}
	...
	t.Run("payload omitting derived fields keeps them", func(t *testing.T) {
		...
		assert.Equal(t, "mailwave-int-1", updated.EmailProvider.SES.ManagedConfigurationSet)
		assert.Equal(t, "mailwave-int-1", updated.EmailProvider.SES.ManagedTenantName)
		assert.Equal(t, "arn:aws:sns:eu-west-3:123456789012:mailwave-ses-int-1", updated.EmailProvider.SES.InboundTopicARN)
	})
...
func TestSESService_ExistingDeploymentUpgradePath(t *testing.T) {
	...
	t.Run("resolves its configuration set by lookup, exactly as before", func(t *testing.T) {
		...
		mockSESv2.EXPECT().SendEmail(gomock.Any(), gomock.Any()).
			DoAndReturn(func(_ context.Context, input *sesv2.SendEmailInput, _ ...func(*sesv2.Options)) (*sesv2.SendEmailOutput, error) {
				assert.Equal(t, "mailwave-int-1", aws.StringValue(input.ConfigurationSetName),
