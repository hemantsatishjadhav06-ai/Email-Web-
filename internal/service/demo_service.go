...
func (s *DemoService) createDemoWorkspace(ctx context.Context) error {
	...
	workspace, err := s.workspaceService.CreateWorkspace(
		...
		"https://demo.mailwave.com/cover.png",
		"UTC",
		fileManagerSettings,
...
func (s *DemoService) createSampleTemplates(ctx context.Context, workspaceID string) error {
	...
	nlMJMLStructures := map[string]mailwave_mjml.EmailBlock{
	...
	nlV2MJMLStructures := map[string]mailwave_mjml.EmailBlock{
	...
	wMJMLStructures := map[string]mailwave_mjml.EmailBlock{
	...
	passwordResetTestData := domain.MapOfAny{
		...
		"reset_url": "https://demo.mailwave.com/reset-password?token=demo_token_123",
	}
	...
	prMJMLStructures := map[string]mailwave_mjml.EmailBlock{
...
// compileTemplateToHTML compiles an MJML structure to HTML using the mailwave_mjml package
func (s *DemoService) compileTemplateToHTML(workspaceID, messageID string, mjmlStructure mailwave_mjml.EmailBlock, testData domain.MapOfAny) string {
	// Convert domain.MapOfAny to mailwave_mjml.MapOfAny
	mjmlTestData := make(mailwave_mjml.MapOfAny)
	for k, v := range testData {
		mjmlTestData[k] = v
	}

	// Create compile request
	compileReq := mailwave_mjml.CompileTemplateRequest{
		WorkspaceID:      workspaceID,
		MessageID:        messageID,
		VisualEditorTree: mjmlStructure,
		TemplateData:     mjmlTestData,
		TrackingSettings: mailwave_mjml.TrackingSettings{
			EnableTracking: false, // Disable tracking for demo templates
		},
	}

	// Compile the template
	resp, err := mailwave_mjml.CompileTemplate(compileReq)
	if err != nil {
...
func (s *DemoService) buildEmailTranslations(
	...
	mjmlStructures map[string]mailwave_mjml.EmailBlock,
...
func getNewsletterContents() map[string]newsletterContent {
	return map[string]newsletterContent{
		"en": {
			...
			footerText: "You received this email because you're subscribed to our newsletter.<br><a href=\"{{unsubscribe_url}}\">Unsubscribe</a> | <a href=\"https://demo.mailwave.com\">Visit our website</a>",
		},
		"fr": {
			lang:       "fr",
			...
			footerText: "Vous recevez cet e-mail car vous êtes abonné(e) à notre newsletter.<br><a href=\"{{unsubscribe_url}}\">Se désabonner</a> | <a href=\"https://demo.mailwave.com\">Visiter notre site</a>",
		},
		"es": {
			lang:       "es",
			...
			footerText: "Recibes este correo porque estás suscrito/a a nuestro boletín.<br><a href=\"{{unsubscribe_url}}\">Cancelar suscripción</a> | <a href=\"https://demo.mailwave.com\">Visitar nuestro sitio</a>",
		},
	}
}
...
func getNewsletterV2Contents() map[string]newsletterV2Content {
	return map[string]newsletterV2Content{
		"en": {
			...
			footerText:      "You're receiving this because you subscribed to our weekly digest.<br><a href=\"{{unsubscribe_url}}\">Unsubscribe</a> | <a href=\"https://demo.mailwave.com/preferences\">Manage Preferences</a>",
		},
		"fr": {
			lang:            "fr",
			...
			footerText:      "Vous recevez ceci car vous êtes abonné(e) à notre résumé hebdomadaire.<br><a href=\"{{unsubscribe_url}}\">Se désabonner</a> | <a href=\"https://demo.mailwave.com/preferences\">Gérer les préférences</a>",
		},
		"es": {
			lang:            "es",
			...
			footerText:      "Recibes esto porque te suscribiste a nuestro resumen semanal.<br><a href=\"{{unsubscribe_url}}\">Cancelar suscripción</a> | <a href=\"https://demo.mailwave.com/preferences\">Gestionar preferencias</a>",
		},
	}
}
...
func (s *DemoService) createNewsletterMJMLStructure(c newsletterContent) mailwave_mjml.EmailBlock {
	...
	headerTextBase := mailwave_mjml.NewBaseBlock("header-text", mailwave_mjml.MJMLComponentMjText)
	headerTextBase.Content = &headerTextContent
	headerText := &mailwave_mjml.MJTextBlock{BaseBlock: headerTextBase}

	// Create main text block
	mainTextBase := mailwave_mjml.NewBaseBlock("main-text", mailwave_mjml.MJMLComponentMjText)
	mainTextBase.Content = &textContent
	mainText := &mailwave_mjml.MJTextBlock{BaseBlock: mainTextBase}

	// Create divider
	divider := &mailwave_mjml.MJDividerBlock{
		BaseBlock: mailwave_mjml.NewBaseBlock("divider", mailwave_mjml.MJMLComponentMjDivider),
	}

	// Create highlights title
	highlightsTextBase := mailwave_mjml.NewBaseBlock("highlights-title", mailwave_mjml.MJMLComponentMjText)
	highlightsTextBase.Content = &highlightsContent
	highlightsText := &mailwave_mjml.MJTextBlock{BaseBlock: highlightsTextBase}

	// Create highlights list
	highlightsListBase := mailwave_mjml.NewBaseBlock("highlights-list", mailwave_mjml.MJMLComponentMjText)
	highlightsListBase.Content = &listContent
	highlightsList := &mailwave_mjml.MJTextBlock{BaseBlock: highlightsListBase}

	// Create button
	buttonBase := mailwave_mjml.NewBaseBlock("cta-button", mailwave_mjml.MJMLComponentMjButton)
	...
	buttonBase.Attributes["href"] = "https://demo.mailwave.com/newsletter?utm_source={{utm_source}}&utm_medium={{utm_medium}}&utm_campaign={{utm_campaign}}"
	buttonBase.Content = &buttonContent
	button := &mailwave_mjml.MJButtonBlock{BaseBlock: buttonBase}

	// Create title and preview blocks
	titleBase := mailwave_mjml.NewBaseBlock("title", mailwave_mjml.MJMLComponentMjTitle)
	titleBase.Content = &titleContent
	title := &mailwave_mjml.MJTitleBlock{BaseBlock: titleBase}

	previewBase := mailwave_mjml.NewBaseBlock("preview", mailwave_mjml.MJMLComponentMjPreview)
	previewBase.Content = &previewContent
	preview := &mailwave_mjml.MJPreviewBlock{BaseBlock: previewBase}

	// Create footer text
	footerContent := c.footerText
	footerTextBase := mailwave_mjml.NewBaseBlock("footer-text", mailwave_mjml.MJMLComponentMjText)
	footerTextBase.Content = &footerContent
	footerText := &mailwave_mjml.MJTextBlock{BaseBlock: footerTextBase}

	// Create columns for layout
	headerColumnBase := mailwave_mjml.NewBaseBlock("header-column", mailwave_mjml.MJMLComponentMjColumn)
	headerColumnBase.Children = []mailwave_mjml.EmailBlock{headerText}
	headerColumn := &mailwave_mjml.MJColumnBlock{BaseBlock: headerColumnBase}

	contentColumnBase := mailwave_mjml.NewBaseBlock("content-column", mailwave_mjml.MJMLComponentMjColumn)
	contentColumnBase.Children = []mailwave_mjml.EmailBlock{mainText, divider, highlightsText, highlightsList, button}
	contentColumn := &mailwave_mjml.MJColumnBlock{BaseBlock: contentColumnBase}

	footerColumnBase := mailwave_mjml.NewBaseBlock("footer-column", mailwave_mjml.MJMLComponentMjColumn)
	footerColumnBase.Children = []mailwave_mjml.EmailBlock{footerText}
	footerColumn := &mailwave_mjml.MJColumnBlock{BaseBlock: footerColumnBase}

	// Create sections
	headerSectionBase := mailwave_mjml.NewBaseBlock("header-section", mailwave_mjml.MJMLComponentMjSection)
	headerSectionBase.Children = []mailwave_mjml.EmailBlock{headerColumn}
	headerSection := &mailwave_mjml.MJSectionBlock{BaseBlock: headerSectionBase}

	contentSectionBase := mailwave_mjml.NewBaseBlock("content-section", mailwave_mjml.MJMLComponentMjSection)
	contentSectionBase.Children = []mailwave_mjml.EmailBlock{contentColumn}
	contentSection := &mailwave_mjml.MJSectionBlock{BaseBlock: contentSectionBase}

	footerSectionBase := mailwave_mjml.NewBaseBlock("footer-section", mailwave_mjml.MJMLComponentMjSection)
	footerSectionBase.Children = []mailwave_mjml.EmailBlock{footerColumn}
	footerSection := &mailwave_mjml.MJSectionBlock{BaseBlock: footerSectionBase}

	// Create head and body
	headBase := mailwave_mjml.NewBaseBlock("head", mailwave_mjml.MJMLComponentMjHead)
	headBase.Children = []mailwave_mjml.EmailBlock{title, preview}
	head := &mailwave_mjml.MJHeadBlock{BaseBlock: headBase}

	bodyBase := mailwave_mjml.NewBaseBlock("body", mailwave_mjml.MJMLComponentMjBody)
	bodyBase.Children = []mailwave_mjml.EmailBlock{headerSection, contentSection, footerSection}
	body := &mailwave_mjml.MJBodyBlock{BaseBlock: bodyBase}

	// Create root MJML block
	rootBase := mailwave_mjml.NewBaseBlock("mjml-root", mailwave_mjml.MJMLComponentMjml)
	rootBase.Attributes["lang"] = c.lang
	rootBase.Children = []mailwave_mjml.EmailBlock{head, body}
...
func (s *DemoService) createNewsletterV2MJMLStructure(c newsletterV2Content) mailwave_mjml.EmailBlock {
	...
	// Create hero section
	heroTextBase := mailwave_mjml.NewBaseBlock("hero-text", mailwave_mjml.MJMLComponentMjText)
	heroTextBase.Content = &heroContent
	heroText := &mailwave_mjml.MJTextBlock{BaseBlock: heroTextBase}

	introTextBase := mailwave_mjml.NewBaseBlock("intro-text", mailwave_mjml.MJMLComponentMjText)
	introTextBase.Content = &introContent
	introText := &mailwave_mjml.MJTextBlock{BaseBlock: introTextBase}

	// Create feature cards
	feature1TitleTextBase := mailwave_mjml.NewBaseBlock("feature1-title", mailwave_mjml.MJMLComponentMjText)
	feature1TitleTextBase.Content = &feature1Title
	feature1TitleText := &mailwave_mjml.MJTextBlock{BaseBlock: feature1TitleTextBase}

	feature1ContentTextBase := mailwave_mjml.NewBaseBlock("feature1-content", mailwave_mjml.MJMLComponentMjText)
	feature1ContentTextBase.Content = &feature1Content
	feature1ContentText := &mailwave_mjml.MJTextBlock{BaseBlock: feature1ContentTextBase}

	feature2TitleTextBase := mailwave_mjml.NewBaseBlock("feature2-title", mailwave_mjml.MJMLComponentMjText)
	feature2TitleTextBase.Content = &feature2Title
	feature2TitleText := &mailwave_mjml.MJTextBlock{BaseBlock: feature2TitleTextBase}

	feature2ContentTextBase := mailwave_mjml.NewBaseBlock("feature2-content", mailwave_mjml.MJMLComponentMjText)
	feature2ContentTextBase.Content = &feature2Content
	feature2ContentText := &mailwave_mjml.MJTextBlock{BaseBlock: feature2ContentTextBase}

	feature3TitleTextBase := mailwave_mjml.NewBaseBlock("feature3-title", mailwave_mjml.MJMLComponentMjText)
	feature3TitleTextBase.Content = &feature3Title
	feature3TitleText := &mailwave_mjml.MJTextBlock{BaseBlock: feature3TitleTextBase}

	feature3ContentTextBase := mailwave_mjml.NewBaseBlock("feature3-content", mailwave_mjml.MJMLComponentMjText)
	feature3ContentTextBase.Content = &feature3Content
	feature3ContentText := &mailwave_mjml.MJTextBlock{BaseBlock: feature3ContentTextBase}

	// Create CTA button
	buttonBase2 := mailwave_mjml.NewBaseBlock("cta-button", mailwave_mjml.MJMLComponentMjButton)
	...
	buttonBase2.Attributes["href"] = "https://demo.mailwave.com/weekly-digest?utm_source={{utm_source}}&utm_medium={{utm_medium}}&utm_campaign={{utm_campaign}}"
	buttonBase2.Content = &buttonContent
	button := &mailwave_mjml.MJButtonBlock{BaseBlock: buttonBase2}
	...
	heroColumnBase := mailwave_mjml.NewBaseBlock("hero-column", mailwave_mjml.MJMLComponentMjColumn)
	heroColumnBase.Children = []mailwave_mjml.EmailBlock{heroText, introText}
	heroColumn := &mailwave_mjml.MJColumnBlock{BaseBlock: heroColumnBase}

	// Create feature columns (side by side layout)
	feature1ColumnBase := mailwave_mjml.NewBaseBlock("feature1-column", mailwave_mjml.MJMLComponentMjColumn)
	feature1ColumnBase.Children = []mailwave_mjml.EmailBlock{feature1TitleText, feature1ContentText}
	feature1Column := &mailwave_mjml.MJColumnBlock{BaseBlock: feature1ColumnBase}

	feature2ColumnBase := mailwave_mjml.NewBaseBlock("feature2-column", mailwave_mjml.MJMLComponentMjColumn)
	feature2ColumnBase.Children = []mailwave_mjml.EmailBlock{feature2TitleText, feature2ContentText}
	feature2Column := &mailwave_mjml.MJColumnBlock{BaseBlock: feature2ColumnBase}

	feature3ColumnBase := mailwave_mjml.NewBaseBlock("feature3-column", mailwave_mjml.MJMLComponentMjColumn)
	feature3ColumnBase.Children = []mailwave_mjml.EmailBlock{feature3TitleText, feature3ContentText}
	feature3Column := &mailwave_mjml.MJColumnBlock{BaseBlock: feature3ColumnBase}

	ctaColumnBase := mailwave_mjml.NewBaseBlock("cta-column", mailwave_mjml.MJMLComponentMjColumn)
	ctaColumnBase.Children = []mailwave_mjml.EmailBlock{button}
	ctaColumn := &mailwave_mjml.MJColumnBlock{BaseBlock: ctaColumnBase}
	...
	// Create sections
	heroSectionBase := mailwave_mjml.NewBaseBlock("hero-section", mailwave_mjml.MJMLComponentMjSection)
	heroSectionBase.Children = []mailwave_mjml.EmailBlock{heroColumn}
	heroSection := &mailwave_mjml.MJSectionBlock{BaseBlock: heroSectionBase}

	featuresSectionBase := mailwave_mjml.NewBaseBlock("features-section", mailwave_mjml.MJMLComponentMjSection)
	featuresSectionBase.Children = []mailwave_mjml.EmailBlock{feature1Column, feature2Column}
	featuresSection := &mailwave_mjml.MJSectionBlock{BaseBlock: featuresSectionBase}

	feature3SectionBase := mailwave_mjml.NewBaseBlock("feature3-section", mailwave_mjml.MJMLComponentMjSection)
	feature3SectionBase.Children = []mailwave_mjml.EmailBlock{feature3Column}
	feature3Section := &mailwave_mjml.MJSectionBlock{BaseBlock: feature3SectionBase}

	ctaSectionBase := mailwave_mjml.NewBaseBlock("cta-section", mailwave_mjml.MJMLComponentMjSection)
	ctaSectionBase.Children = []mailwave_mjml.EmailBlock{ctaColumn}
	ctaSection := &mailwave_mjml.MJSectionBlock{BaseBlock: ctaSectionBase}
	...
	bodyBase := mailwave_mjml.NewBaseBlock("body", mailwave_mjml.MJMLComponentMjBody)
	bodyBase.Children = []mailwave_mjml.EmailBlock{heroSection, featuresSection, feature3Section, ctaSection, footerSection}
	body := &mailwave_mjml.MJBodyBlock{BaseBlock: bodyBase}
...
func (s *DemoService) createWelcomeMJMLStructure(c welcomeContent) mailwave_mjml.EmailBlock {
	...
	welcomeTextBase := mailwave_mjml.NewBaseBlock("welcome-text", mailwave_mjml.MJMLComponentMjText)
	welcomeTextBase.Content = &welcomeText
	welcomeTextBlock := &mailwave_mjml.MJTextBlock{BaseBlock: welcomeTextBase}

	mainTextBase := mailwave_mjml.NewBaseBlock("main-text", mailwave_mjml.MJMLComponentMjText)
	mainTextBase.Content = &mainContentText
	mainText := &mailwave_mjml.MJTextBlock{BaseBlock: mainTextBase}

	buttonBase3 := mailwave_mjml.NewBaseBlock("get-started-button", mailwave_mjml.MJMLComponentMjButton)
	...
	buttonBase3.Attributes["href"] = "https://demo.mailwave.com/getting-started?utm_source={{utm_source}}&utm_medium={{utm_medium}}&utm_campaign={{utm_campaign}}"
	buttonBase3.Content = &buttonContent
	button := &mailwave_mjml.MJButtonBlock{BaseBlock: buttonBase3}
	...
	columnBase := mailwave_mjml.NewBaseBlock("main-column", mailwave_mjml.MJMLComponentMjColumn)
	columnBase.Children = []mailwave_mjml.EmailBlock{welcomeTextBlock, mainText, button, divider, footerText}
	column := &mailwave_mjml.MJColumnBlock{BaseBlock: columnBase}

	sectionBase := mailwave_mjml.NewBaseBlock("main-section", mailwave_mjml.MJMLComponentMjSection)
	sectionBase.Children = []mailwave_mjml.EmailBlock{column}
	section := &mailwave_mjml.MJSectionBlock{BaseBlock: sectionBase}
	...
	bodyBase := mailwave_mjml.NewBaseBlock("body", mailwave_mjml.MJMLComponentMjBody)
	bodyBase.Children = []mailwave_mjml.EmailBlock{section}
	body := &mailwave_mjml.MJBodyBlock{BaseBlock: bodyBase}

	rootBase7 := mailwave_mjml.NewBaseBlock("mjml-root", mailwave_mjml.MJMLComponentMjml)
	rootBase7.Attributes["lang"] = c.lang
	rootBase7.Children = []mailwave_mjml.EmailBlock{head, body}
	return &mailwave_mjml.MJMLBlock{BaseBlock: rootBase7}
}

// demoBroadcastUTMSource is the utm_source the sample broadcasts tag their
// links with.
const demoBroadcastUTMSource = "demo.mailwave.com"
...
func (s *DemoService) createPasswordResetMJMLStructure(c passwordResetContent) mailwave_mjml.EmailBlock {
	// Create content strings
	titleContent := c.title
	previewContent := c.preview
	...
	buttonBase4 := mailwave_mjml.NewBaseBlock("reset-button", mailwave_mjml.MJMLComponentMjButton)
	...
	button := &mailwave_mjml.MJButtonBlock{BaseBlock: buttonBase4}

	expireTextBase := mailwave_mjml.NewBaseBlock("expire-text", mailwave_mjml.MJMLComponentMjText)
	expireTextBase.Content = &expireContent
	expireText := &mailwave_mjml.MJTextBlock{BaseBlock: expireTextBase}
	...
	columnBase := mailwave_mjml.NewBaseBlock("main-column", mailwave_mjml.MJMLComponentMjColumn)
	columnBase.Children = []mailwave_mjml.EmailBlock{headerText, mainText, button, expireText, divider, footerText}
	column := &mailwave_mjml.MJColumnBlock{BaseBlock: columnBase}
	...
	rootBase6 := mailwave_mjml.NewBaseBlock("mjml-root", mailwave_mjml.MJMLComponentMjml)
	rootBase6.Attributes["lang"] = c.lang
	rootBase6.Children = []mailwave_mjml.EmailBlock{head, body}
	return &mailwave_mjml.MJMLBlock{BaseBlock: rootBase6}
}
...
func (s *DemoService) generateTransactionalMessageHistoryForContact(contact *domain.Contact, templateID string, templateVersion int64, messageType string, baseTime time.Time) (*domain.MessageHistory, messageEngagement) {
	...
	if messageType == "password-reset" {
		messageData.Data["reset_url"] = "https://demo.mailwave.com/reset-password?token=demo_token_123"
	}
