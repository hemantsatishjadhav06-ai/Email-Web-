...
import (
	...
	domainmocks "github.com/Mailwave/mailwave/internal/domain/mocks" // Corrected import path
	"github.com/Mailwave/mailwave/internal/service"                  // Added logger import
	"github.com/Mailwave/mailwave/pkg/logger"
	pkgmocks "github.com/Mailwave/mailwave/pkg/mocks" // Corrected import path
...
func TestTemplateService_CreateTemplate(t *testing.T) {
	...
	templateToCreate := &domain.Template{
		...
		Email: &domain.EmailTemplate{
			...
			VisualEditorTree: func() mailwave_mjml.EmailBlock {
...
func createTestTextBlock(id, textContent string) mailwave_mjml.EmailBlock {
	content := textContent
	base := mailwave_mjml.NewBaseBlock(id, mailwave_mjml.MJMLComponentMjText)
	base.Content = &content
	return &mailwave_mjml.MJTextBlock{BaseBlock: base}
}

// --- Helper to create a valid nested structure for testing success ---
func createValidTestTree(textBlock mailwave_mjml.EmailBlock) mailwave_mjml.EmailBlock {
	columnBase := mailwave_mjml.NewBaseBlock("col1", mailwave_mjml.MJMLComponentMjColumn)
	columnBase.Children = []mailwave_mjml.EmailBlock{textBlock}
	columnBlock := &mailwave_mjml.MJColumnBlock{BaseBlock: columnBase}

	sectionBase := mailwave_mjml.NewBaseBlock("sec1", mailwave_mjml.MJMLComponentMjSection)
	sectionBase.Children = []mailwave_mjml.EmailBlock{columnBlock}
	sectionBlock := &mailwave_mjml.MJSectionBlock{BaseBlock: sectionBase}

	bodyBase := mailwave_mjml.NewBaseBlock("body1", mailwave_mjml.MJMLComponentMjBody)
	bodyBase.Children = []mailwave_mjml.EmailBlock{sectionBlock}
	bodyBlock := &mailwave_mjml.MJBodyBlock{BaseBlock: bodyBase}
...
func TestCompileTemplate_Success(t *testing.T) {
	...
	testData := mailwave_mjml.MapOfAny{"name": "Tester"}
...
func TestCompileTemplate_TreeToMjmlError(t *testing.T) {
	...
	badLiquidBase := mailwave_mjml.NewBaseBlock("badliq", mailwave_mjml.MJMLComponentMjText)
	badLiquidBase.Content = &invalidContent
	badLiquidBlock := &mailwave_mjml.MJTextBlock{BaseBlock: badLiquidBase}
	...
	resp, err := svc.CompileTemplate(ctx, domain.CompileTemplateRequest{
		...
		TemplateData:     mailwave_mjml.MapOfAny{"name": "test"}, // Provide template data to trigger liquid processing
	})
...
func TestCompileTemplate_InvalidTreeData(t *testing.T) {
	...
	workspaceID := "ws_123"
	userID := "user_abc"
	invalidTree := &mailwave_mjml.MJMLBlock{
		BaseBlock: mailwave_mjml.NewBaseBlock("root_invalid", mailwave_mjml.MJMLComponentMjml),
	}
...
func TestCompileTemplate_InjectsWorkspaceData(t *testing.T) {
	...
	resp, err := svc.CompileTemplate(ctx, domain.CompileTemplateRequest{
		WorkspaceID:      workspaceID,
		VisualEditorTree: testTree,
		TemplateData:     mailwave_mjml.MapOfAny{},
	})
...
func TestCompileTemplate_WorkspaceBaseURLFallsBackToAPIEndpoint(t *testing.T) {
	...
	resp, err := svc.CompileTemplate(ctx, domain.CompileTemplateRequest{
		...
		TrackingSettings: mailwave_mjml.TrackingSettings{Endpoint: "https://other.example.com"},
	})
...
func TestCompileTemplate_WorkspaceLoadFailureUsesFallback(t *testing.T) {
	...
	resp, err := svc.CompileTemplate(ctx, domain.CompileTemplateRequest{
		...
		VisualEditorTree: testTree,
		TemplateData:     mailwave_mjml.MapOfAny{},
		TrackingSettings: mailwave_mjml.TrackingSettings{Endpoint: "https://fallback.example.com"},
	})
...
// also works when the provided workspace object is a mailwave_mjml.MapOfAny (built in Go)
...
func TestCompileTemplate_InjectsOverNonMapWorkspace(t *testing.T) {
	...
	resp, err := svc.CompileTemplate(ctx, domain.CompileTemplateRequest{
		...
		TemplateData: mailwave_mjml.MapOfAny{"workspace": nil},
	})
...
func TestTemplateService_UpdateEmailMetadataBlocks_Translations(t *testing.T) {
  ...
  <mj-body>
	...
	t.Run("CreateTemplate stamps mj-preview for visual-mode translation", func(t *testing.T) {
		...
		buildTreeWithStalePreview := func(stale string) mailwave_mjml.EmailBlock {
			previewBase := mailwave_mjml.NewBaseBlock("preview", mailwave_mjml.MJMLComponentMjPreview)
			previewBase.Content = &stale
			previewBlock := &mailwave_mjml.MJPreviewBlock{BaseBlock: previewBase}
			headBase := mailwave_mjml.NewBaseBlock("head", mailwave_mjml.MJMLComponentMjHead)
			headBase.Children = []mailwave_mjml.EmailBlock{previewBlock}
			headBlock := &mailwave_mjml.MJHeadBlock{BaseBlock: headBase}
			bodyBase := mailwave_mjml.NewBaseBlock("body", mailwave_mjml.MJMLComponentMjBody)
			bodyBlock := &mailwave_mjml.MJBodyBlock{BaseBlock: bodyBase}
			rootBase := mailwave_mjml.NewBaseBlock("root", mailwave_mjml.MJMLComponentMjml)
			rootBase.Children = []mailwave_mjml.EmailBlock{headBlock, bodyBlock}
			return &mailwave_mjml.MJMLBlock{BaseBlock: rootBase}
		}
		...
		mockRepo.EXPECT().CreateTemplate(ctx, workspaceID, gomock.Any()).DoAndReturn(
			func(_ context.Context, _ string, tmplArg *domain.Template) error {
				fr := tmplArg.Translations["fr"]
				require.NotNil(t, fr.Email)
				frMjml := mailwave_mjml.ConvertJSONToMJML(fr.Email.VisualEditorTree)
...
func TestTemplateService_UpdateTemplate_AbsentFields(t *testing.T) {
	...
	treeWithTitle := func(title string) mailwave_mjml.EmailBlock {
		titleBase := mailwave_mjml.NewBaseBlock("title", mailwave_mjml.MJMLComponentMjTitle)
		titleBase.Content = &title
		titleBlock := &mailwave_mjml.MJTitleBlock{BaseBlock: titleBase}
		headBase := mailwave_mjml.NewBaseBlock("head", mailwave_mjml.MJMLComponentMjHead)
		headBase.Children = []mailwave_mjml.EmailBlock{titleBlock}
		headBlock := &mailwave_mjml.MJHeadBlock{BaseBlock: headBase}
		bodyBase := mailwave_mjml.NewBaseBlock("body", mailwave_mjml.MJMLComponentMjBody)
		bodyBlock := &mailwave_mjml.MJBodyBlock{BaseBlock: bodyBase}
		rootBase := mailwave_mjml.NewBaseBlock("root", mailwave_mjml.MJMLComponentMjml)
	...
	t.Run("omitted translations are preserved and restamped with the new name", func(t *testing.T) {
		...
		mockRepo.EXPECT().UpdateTemplate(ctx, workspaceID, gomock.Any(), gomock.Any()).DoAndReturn(
			func(_ context.Context, _ string, tmpl *domain.Template, _ int64) error {
				...
				assert.Contains(t, mailwave_mjml.ConvertJSONToMJML(fr.Email.VisualEditorTree), "<mj-title>New Name</mj-title>")
				return nil
			},
		)
