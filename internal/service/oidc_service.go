...
// OIDCService mints Mailwave sessions from a verified external OIDC identity. It is
...
func (s *OIDCService) resolveOrProvisionUser(
	...
	// (2) Existing Mailwave user by email (invited-user bridge) — CASE-INSENSITIVE.
	...
	if !s.cfg.AutoCreateUsers {
		if s.logger != nil {
			s.logger.WithField("email", email).WithField("oidc_sub", sub).
				Warn("OIDC login rejected: no Mailwave account (invited-only)")
		}
		return nil, domain.ErrOIDCAccountNotProvisioned
	}
