...
func (s *PostmarkService) RegisterWebhooks(
	...
	mailwaveWebhooks := s.filterPostmarkWebhooks(existingWebhooks.Webhooks, baseURL, workspaceID, integrationID)

	// If we have existing webhooks, unregister them
	for _, webhook := range mailwaveWebhooks {
...
func (s *PostmarkService) GetWebhookStatus(
	...
	mailwaveWebhooks := s.filterPostmarkWebhooks(existingWebhooks.Webhooks, "", workspaceID, integrationID)
...
func (s *PostmarkService) SendEmail(ctx context.Context, request domain.SendEmailProviderRequest) error {
	...
	requestBody := map[string]interface{}{
		...
		"Metadata": map[string]string{
			"mailwave_message_id": request.MessageID,
		},
	}
