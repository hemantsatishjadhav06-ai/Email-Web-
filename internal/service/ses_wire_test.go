...
func TestSESWire_SimpleSendSerialisesCorrectly(t *testing.T) {
	...
	assert.Equal(t, "mailwave-int-1", body["ConfigurationSetName"])
	assert.Equal(t, "mailwave-int-1", body["TenantName"])
	...
	assert.Equal(t, "mailwave_message_id", tags[0].(map[string]interface{})["Name"])
}
