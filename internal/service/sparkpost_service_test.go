	...
	t.Run("Success", func(t *testing.T) {
		...
		mockHTTPClient.EXPECT().
			...
			DoAndReturn(func(req *http.Request) (*http.Response, error) {
				...
				assert.True(t, ok)
				assert.Equal(t, "test-message-id", metadata["mailwave_message_id"])
		...
		gomock.InOrder(
			...
			mockHTTPClient.EXPECT().
				...
				DoAndReturn(func(req *http.Request) (*http.Response, error) {
					...
					var webhook domain.SparkPostWebhook
					_ = json.Unmarshal(body, &webhook)
					assert.Equal(t, "Mailwave-very-long-integration-id-that-exceeds-limit", webhook.Name)
					return mockHTTPResponse(http.StatusOK, string(createResponseJSON)), nil
				}),
		)
		...
		gomock.InOrder(
			...
			mockHTTPClient.EXPECT().
				...
				DoAndReturn(func(req *http.Request) (*http.Response, error) {
					...
					var webhook domain.SparkPostWebhook
					_ = json.Unmarshal(body, &webhook)
					assert.Equal(t, "Mailwave-integration", webhook.Name)
					return mockHTTPResponse(http.StatusOK, string(createResponseJSON)), nil
				}),
		)
