...
import (
	...
	"github.com/Mailwave/mailwave/pkg/safehttpclient"
...
func TestProcessSESWebhook(t *testing.T) {
	...
	t.Run("Bounce Event with Mailwave Message ID", func(t *testing.T) {
		// Create test bounce payload with mailwave_message_id in tags
		payload := domain.SESWebhookPayload{
			Message: `{"eventType":"Bounce","bounce":{"bounceType":"Permanent","bounceSubType":"General","bouncedRecipients":[{"emailAddress":"test@example.com","diagnosticCode":"554"}],"timestamp":"2023-01-01T12:00:00Z"},"mail":{"messageId":"provider-msg-id","tags":{"mailwave_message_id":["mailwave-123"]}}}`,
		}
		rawPayload, err := json.Marshal(payload)
		require.NoError(t, err)
		...
		// Assert - should use mailwave message ID
		assert.NoError(t, err)
		assert.NotNil(t, events)
		assert.NotNil(t, events[0].MessageID)
		assert.Equal(t, "mailwave-123", *events[0].MessageID)
	})

	t.Run("Delivery Event with Mailwave Message ID", func(t *testing.T) {
		// Create test delivery payload with mailwave_message_id in tags
		payload := domain.SESWebhookPayload{
			Message: `{"eventType":"Delivery","delivery":{"recipients":["test@example.com"],"timestamp":"2023-01-01T12:00:00Z"},"mail":{"messageId":"provider-msg-id","tags":{"mailwave_message_id":["mailwave-456"]}}}`,
		}
		rawPayload, err := json.Marshal(payload)
		require.NoError(t, err)
		...
		assert.NotNil(t, events)
		assert.NotNil(t, events[0].MessageID)
		assert.Equal(t, "mailwave-456", *events[0].MessageID)
	})

	t.Run("Complaint Event with Mailwave Message ID", func(t *testing.T) {
		// Create test complaint payload with mailwave_message_id in tags
		payload := domain.SESWebhookPayload{
			Message: `{"eventType":"Complaint","complaint":{"complainedRecipients":[{"emailAddress":"test@example.com"}],"timestamp":"2023-01-01T12:00:00Z","complaintFeedbackType":"abuse"},"mail":{"messageId":"provider-msg-id","tags":{"mailwave_message_id":["mailwave-789"]}}}`,
		}
		rawPayload, err := json.Marshal(payload)
		require.NoError(t, err)
		...
		assert.NotNil(t, events)
		assert.NotNil(t, events[0].MessageID)
		assert.Equal(t, "mailwave-789", *events[0].MessageID)
	})
	...
	t.Run("Bounce Event with X-Message-ID Header Fallback", func(t *testing.T) {
		...
		payload := domain.SESWebhookPayload{
			Message: `{"eventType":"Bounce","bounce":{"bounceType":"Permanent","bounceSubType":"General","bouncedRecipients":[{"emailAddress":"test@example.com","diagnosticCode":"554"}],"timestamp":"2023-01-01T12:00:00Z"},"mail":{"messageId":"provider-msg-id","headers":[{"name":"X-Message-ID","value":"mailwave-header-123"}]}}`,
		}
		rawPayload, err := json.Marshal(payload)
		require.NoError(t, err)
		...
		assert.Len(t, events, 1)
		assert.NotNil(t, events[0].MessageID)
		assert.Equal(t, "mailwave-header-123", *events[0].MessageID)
	})
	...
	t.Run("Delivery Event with X-Message-ID Header Fallback", func(t *testing.T) {
		...
		payload := domain.SESWebhookPayload{
			Message: `{"eventType":"Delivery","delivery":{"recipients":["test@example.com"],"timestamp":"2023-01-01T12:00:00Z"},"mail":{"messageId":"provider-msg-id","headers":[{"name":"X-Message-ID","value":"mailwave-header-456"}]}}`,
		}
		rawPayload, err := json.Marshal(payload)
		require.NoError(t, err)
		...
		assert.Len(t, events, 1)
		assert.NotNil(t, events[0].MessageID)
		assert.Equal(t, "mailwave-header-456", *events[0].MessageID)
	})
	...
	t.Run("Complaint Event with X-Message-ID Header Fallback", func(t *testing.T) {
		...
		payload := domain.SESWebhookPayload{
			Message: `{"eventType":"Complaint","complaint":{"complainedRecipients":[{"emailAddress":"test@example.com"}],"timestamp":"2023-01-01T12:00:00Z","complaintFeedbackType":"abuse"},"mail":{"messageId":"provider-msg-id","headers":[{"name":"X-Message-ID","value":"mailwave-header-789"}]}}`,
		}
		rawPayload, err := json.Marshal(payload)
		require.NoError(t, err)
		...
		assert.Len(t, events, 1)
		assert.NotNil(t, events[0].MessageID)
		assert.Equal(t, "mailwave-header-789", *events[0].MessageID)
	})
	...
	t.Run("Tags take priority over X-Message-ID header fallback", func(t *testing.T) {
		...
		payload := domain.SESWebhookPayload{
			Message: `{"eventType":"Delivery","delivery":{"recipients":["test@example.com"],"timestamp":"2023-01-01T12:00:00Z"},"mail":{"messageId":"provider-msg-id","tags":{"mailwave_message_id":["tag-message-id"]},"headers":[{"name":"X-Message-ID","value":"header-message-id"}]}}`,
		}
		rawPayload, err := json.Marshal(payload)
		require.NoError(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			headers: []domain.SESHeader{
				...
				{Name: "X-Message-ID", Value: "mailwave-123"},
				{Name: "Subject", Value: "Test"},
			},
			expected: "mailwave-123",
		},
		{
			name: "X-Message-ID with different case",
			headers: []domain.SESHeader{
				{Name: "x-message-id", Value: "mailwave-456"},
			},
			expected: "mailwave-456",
		},
		{
...
func TestProcessPostmarkWebhook(t *testing.T) {
	...
	t.Run("Event with Mailwave Message ID in Metadata", func(t *testing.T) {
		// Create test delivery payload with mailwave_message_id in metadata
		rawPayload, err := json.Marshal(map[string]interface{}{
			"RecordType":  "Delivery",
			...
			"Metadata": map[string]interface{}{
				"mailwave_message_id": "mailwave-123",
			},
		})
		require.NoError(t, err)
		...
		assert.NoError(t, err)
		assert.NotNil(t, events)
		// Should use mailwave message ID instead of provider message ID
		assert.NotNil(t, events[0].MessageID)
		assert.Equal(t, "mailwave-123", *events[0].MessageID)
	})
...
func TestProcessSparkPostWebhook(t *testing.T) {
	...
	t.Run("Event with Mailwave Message ID", func(t *testing.T) {
		// Create test delivery payload with mailwave_message_id in recipient meta
		payload := []*domain.SparkPostWebhookPayload{
			{
				MSys: domain.SparkPostMSys{
					MessageEvent: &domain.SparkPostMessageEvent{
...
func TestProcessMailgunWebhook(t *testing.T) {
	...
	t.Run("Event with Mailwave Message ID in User Variables", func(t *testing.T) {
		// Create test delivery payload with mailwave_message_id in user variables
...
func TestProcessSendGridWebhook(t *testing.T) {
	...
	t.Run("Delivered Event", func(t *testing.T) {
		...
		assert.Equal(t, "test@example.com", events[0].RecipientEmail)
		assert.NotNil(t, events[0].MessageID)
		assert.Equal(t, "msg_123", *events[0].MessageID) // Uses mailwave_message_id
	})

	t.Run("Delivered Event - Fallback to sg_message_id", func(t *testing.T) {
		// When mailwave_message_id is not present, fall back to sg_message_id
		rawPayload := []byte(`[{
			"email": "test@example.com",
			"timestamp": 1706097600,
			"event": "delivered",
		...
		rawPayload := []byte(`[
			{
				...
				"sg_event_id": "event1",
				"sg_message_id": "msg1",
				"mailwave_message_id": "mailwave_1"
			},
			{
				...
				"reason": "User unknown",
				"bounce_classification": "Invalid Address",
				"mailwave_message_id": "mailwave_2"
			},
			{
				...
				"sg_event_id": "event3",
				"sg_message_id": "msg3",
				"mailwave_message_id": "mailwave_3"
			}
		]`)
