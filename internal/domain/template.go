...
import (
	...
	// Import the mailwave_mjml package

	"github.com/Mailwave/mailwave/pkg/mailwave_mjml"
	"github.com/asaskevich/govalidator"
)

//go:generate mockgen -destination mocks/mock_template_service.go -package mocks github.com/Mailwave/mailwave/internal/domain TemplateService
//go:generate mockgen -destination mocks/mock_template_repository.go -package mocks github.com/Mailwave/mailwave/internal/domain TemplateRepository
...
type EmailTemplate struct {
	...
	VisualEditorTree mailwave_mjml.EmailBlock `json:"visual_editor_tree"`
...
func (e *EmailTemplate) Validate(testData MapOfAny) error {
	...
	if e.EditorMode == EditorModeCode {
		...
		if e.VisualEditorTree.GetType() != mailwave_mjml.MJMLComponentMjml {
		...
		if e.CompiledPreview == "" {
			...
			// Compile tree to MJML using our pkg/mailwave_mjml function
			var mjmlResult string
			if templateDataStr != "" {
				result, err := mailwave_mjml.ConvertJSONToMJMLWithData(e.VisualEditorTree, templateDataStr)
				if err != nil {
					return fmt.Errorf("failed to convert tree to MJML: %w", err)
				}
				mjmlResult = result
			} else {
				mjmlResult = mailwave_mjml.ConvertJSONToMJML(e.VisualEditorTree)
			}
...
func (x *EmailTemplate) UnmarshalJSON(data []byte) error {
	...
	if len(aux.VisualEditorTree) > 0 {
		block, err := mailwave_mjml.UnmarshalEmailBlock(aux.VisualEditorTree)
		if err != nil {
...
// Use types from mailwave_mjml package
type CompileTemplateRequest = mailwave_mjml.CompileTemplateRequest
type CompileTemplateResponse = mailwave_mjml.CompileTemplateResponse
...
type TemplateService interface {
	...
	CompileTemplate(ctx context.Context, payload CompileTemplateRequest) (*CompileTemplateResponse, error) // Use mailwave_mjml.EmailBlock
}
...
type TemplateDataRequest struct {
	...
	TrackingSettings    mailwave_mjml.TrackingSettings `json:"tracking_settings"`
...
func BuildTemplateData(req TemplateDataRequest) (MapOfAny, error) {
	...
	//     used for unsubscribe/tracking/notification-center links on the Mailwave domain.
