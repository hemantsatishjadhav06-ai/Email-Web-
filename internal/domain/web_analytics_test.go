...
func TestWebAnalyticsSettings(t *testing.T) {
	...
	// The matching loop is shared with mailwave_mjml.MatchesAllowedHost, which
	...
	// token" in mailwave_mjml.MatchesAllowedHost. Sharing the loop must not have
...
func TestResolveWebIdentity(t *testing.T) {
	...
	t.Run("the analytics HMAC is domain-separated from the subscription one", func(t *testing.T) {
		...
		// every email Mailwave sends. If the two were interchangeable, an
