...
func createValidMJMLBlock() mailwave_mjml.EmailBlock {
	bodyBlock := &mailwave_mjml.MJBodyBlock{
		BaseBlock: mailwave_mjml.NewBaseBlock("body-1", mailwave_mjml.MJMLComponentMjBody),
	}
	bodyBlock.Children = []mailwave_mjml.EmailBlock{}

	mjmlBlock := &mailwave_mjml.MJMLBlock{
		BaseBlock: mailwave_mjml.NewBaseBlock("mjml-root", mailwave_mjml.MJMLComponentMjml),
	}
	mjmlBlock.Attributes["lang"] = "en"
	mjmlBlock.Children = []mailwave_mjml.EmailBlock{bodyBlock}

	return mjmlBlock
}

// createInvalidMJMLBlock creates an invalid MJML EmailBlock for testing EmailTemplate
func createInvalidMJMLBlock(blockType mailwave_mjml.MJMLComponentType) mailwave_mjml.EmailBlock {
	return &mailwave_mjml.MJTextBlock{
		BaseBlock: mailwave_mjml.NewBaseBlock("text-1", blockType),
	}
}
		...
		},
		{
			...
			template: func() *EmailTemplate {
				e := &EmailTemplate{
					...
					Subject:          "Test Subject",
					CompiledPreview:  "",
					VisualEditorTree: createInvalidMJMLBlock(mailwave_mjml.MJMLComponentMjml),
				}
				return e
			}(),
			testData: nil,
		...
		},
		{
			...
			template: func() *EmailTemplate {
				e := &EmailTemplate{
					...
					Subject:          "Test Subject",
					CompiledPreview:  "<html>Test content</html>",
					VisualEditorTree: createInvalidMJMLBlock(mailwave_mjml.MJMLComponentMjText),
				}
				return e
			}(),
			testData: nil,
...
func TestBuildTemplateData(t *testing.T) {
	...
	t.Run("workspace base url reflects custom endpoint and trims trailing slash", func(t *testing.T) {
		...
		req := TemplateDataRequest{
			...
			WorkspaceSecretKey: "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
			MessageID:          "msg-456",
			TrackingSettings:   mailwave_mjml.TrackingSettings{Endpoint: "https://links.example.com/"},
		}
		data, err := BuildTemplateData(req)
...
// switched off — that is how a client supplies its own payload instead of having Mailwave fetch
...
func TestBuildTemplateData_GlobalFeed(t *testing.T) {
	...
	newReq := func(feed *GlobalFeedSettings, data MapOfAny) TemplateDataRequest {
		return TemplateDataRequest{
			...
			MessageID:          "msg-456",
			TrackingSettings:   mailwave_mjml.TrackingSettings{Endpoint: "https://api.example.com"},
			Broadcast: &Broadcast{
				ID:   "broadcast-001",
				Name: "Test Broadcast",
...
func TestGenerateEmailRedirectionEndpoint(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			url := mailwave_mjml.GenerateEmailRedirectionEndpoint(tt.workspaceID, tt.messageID, tt.apiEndpoint, tt.destinationURL, testTimestamp)
...
func TestEmailTemplate_MjRawContent_JSONRoundTrip(t *testing.T) {
	...
	assert.Equal(t, mailwave_mjml.MJMLComponentMjml, emailTemplate.VisualEditorTree.GetType())

	// Find the mj-raw block and verify content
	var rawBlock mailwave_mjml.EmailBlock
	...
	assert.Equal(t, mailwave_mjml.MJMLComponentMjRaw, rawBlock.GetType(), "Expected mj-raw block")
	...
	var rawBlock2 mailwave_mjml.EmailBlock
...
func TestEmailTemplate_MjRawContent_Value_Scan(t *testing.T) {
	...
	rawBase := mailwave_mjml.NewBaseBlock("raw-1", mailwave_mjml.MJMLComponentMjRaw)
	rawBase.Content = &rawContent
	rawBlock := &mailwave_mjml.MJRawBlock{BaseBlock: rawBase}

	columnBlock := &mailwave_mjml.MJColumnBlock{BaseBlock: mailwave_mjml.NewBaseBlock("column-1", mailwave_mjml.MJMLComponentMjColumn)}
	columnBlock.Children = []mailwave_mjml.EmailBlock{rawBlock}

	sectionBlock := &mailwave_mjml.MJSectionBlock{BaseBlock: mailwave_mjml.NewBaseBlock("section-1", mailwave_mjml.MJMLComponentMjSection)}
	sectionBlock.Children = []mailwave_mjml.EmailBlock{columnBlock}

	bodyBlock := &mailwave_mjml.MJBodyBlock{BaseBlock: mailwave_mjml.NewBaseBlock("body-1", mailwave_mjml.MJMLComponentMjBody)}
	bodyBlock.Children = []mailwave_mjml.EmailBlock{sectionBlock}

	mjmlBlock := &mailwave_mjml.MJMLBlock{BaseBlock: mailwave_mjml.NewBaseBlock("mjml-1", mailwave_mjml.MJMLComponentMjml)}
	mjmlBlock.Children = []mailwave_mjml.EmailBlock{bodyBlock}
	...
	assert.Equal(t, mailwave_mjml.MJMLComponentMjRaw, rawBlock2.GetType(), "Expected mj-raw block after Scan")
	content2 := rawBlock2.GetContent()
