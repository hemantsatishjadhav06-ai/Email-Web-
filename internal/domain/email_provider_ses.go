...
//go:generate mockgen -destination mocks/mock_ses_service.go -package mocks github.com/Mailwave/mailwave/internal/domain SESServiceInterface
...
type AmazonSESSettings struct {
	...
	// TenantIsolationEnabled turns on Mailwave-managed SES tenant isolation: a tenant per
	...
	// ConfigurationSetName overrides the auto-managed "mailwave-<integrationID>" configuration
	// set. Empty means Mailwave manages it.
	...
	// ManagedConfigurationSet is the "mailwave-<integrationID>" set created during webhook
	...
	// ManagedTenantName is the tenant Mailwave created and associated. Empty while isolation is
...
// first, then the set Mailwave manages. An empty result means "none resolved" — the caller decides
...
// then the one Mailwave manages — and that one only while isolation is actually switched on.
//
...
// OwnsManagedTenant reports whether Mailwave created the tenant, and may therefore delete it.
