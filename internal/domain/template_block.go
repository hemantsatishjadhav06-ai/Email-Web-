...
//go:generate mockgen -destination mocks/mock_template_block_service.go -package mocks github.com/Mailwave/mailwave/internal/domain TemplateBlockService

// TemplateBlock represents a reusable email block template
type TemplateBlock struct {
	ID      string                   `json:"id"`
	Name    string                   `json:"name"`
	Block   mailwave_mjml.EmailBlock `json:"block"`
...
func (tb *TemplateBlock) UnmarshalJSON(data []byte) error {
	...
	if len(temp.Block) > 0 {
		...
		if blockStr == `""` || blockStr == `null` {
			...
			block, err := mailwave_mjml.UnmarshalEmailBlock(temp.Block)
			if err != nil {
