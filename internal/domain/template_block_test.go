...
func createTestEmailBlock() mailwave_mjml.EmailBlock {
	blockJSON := []byte(`{"id":"b1","type":"mj-text","content":"Hello","attributes":{"fontSize":"16px"}}`)
	blk, _ := mailwave_mjml.UnmarshalEmailBlock(blockJSON)
	return blk
}
...
func (d dummyEmptyTypeBlock) GetType() mailwave_mjml.MJMLComponentType        { return "" }
func (d dummyEmptyTypeBlock) GetChildren() []mailwave_mjml.EmailBlock         { return nil }
func (d dummyEmptyTypeBlock) SetChildren(children []mailwave_mjml.EmailBlock) {}
	...
	tests := []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, tb *TemplateBlock) {
				...
				assert.Equal(t, mailwave_mjml.MJMLComponentMjText, tb.Block.GetType())
			},
		},
		{
			name: "null block",
