...
func TestAmazonSESSettings_Validate_SendingContext(t *testing.T) {
	...
	t.Run("valid names accepted", func(t *testing.T) {
		s := base()
		s.ConfigurationSetName = "mailwave-a3f1_set-1"
...
func TestAmazonSESSettings_JSONRoundTrip_OmitsEmptySendingContext(t *testing.T) {
	...
	t.Run("stored blob unmarshals intact", func(t *testing.T) {
		raw := `{
			...
			"managed_configuration_set": "mailwave-abc",
			"managed_tenant_name": "mailwave-abc"
		}`
		...
		assert.Equal(t, "mailwave-abc", s.ManagedConfigurationSet)
		assert.Equal(t, "mailwave-abc", s.ManagedTenantName)
	})
}
...
func TestAmazonSESSettings_Resolution(t *testing.T) {
	t.Run("override wins over managed", func(t *testing.T) {
		s := domain.AmazonSESSettings{
			...
			ManagedConfigurationSet: "mailwave-abc",
			TenantName:              "team-acme",
			ManagedTenantName:       "mailwave-abc",
		}
	...
	t.Run("managed used when no override and isolation is on", func(t *testing.T) {
		...
		assert.Equal(t, "mailwave-abc", s.ResolveConfigurationSet())
		assert.Equal(t, "mailwave-abc", s.ResolveTenant())
	})
	...
	t.Run("switching isolation off stops sends using the managed tenant", func(t *testing.T) {
		...
		assert.Equal(t, "mailwave-abc", s.KnownTenant())
...
func TestSESResourceARN(t *testing.T) {
	tenantARN := "arn:aws:ses:eu-west-3:123456789012:tenant/mailwave-abc/t-1"

	assert.Equal(t,
		"arn:aws:ses:eu-west-3:123456789012:configuration-set/mailwave-abc",
		domain.SESResourceARN(tenantARN, "configuration-set", "mailwave-abc"))
...
func TestSESResourceNameFromARN(t *testing.T) {
	assert.Equal(t, "mailwave-abc",
		domain.SESResourceNameFromARN("arn:aws:ses:eu-west-3:123456789012:configuration-set/mailwave-abc"))

	// A suffix comparison would call this a match for "mailwave-abc"; a parsed one does not.
	assert.Equal(t, "prod-mailwave-abc",
		domain.SESResourceNameFromARN("arn:aws:ses:eu-west-3:123456789012:configuration-set/prod-mailwave-abc"))
