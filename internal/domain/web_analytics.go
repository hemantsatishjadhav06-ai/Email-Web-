...
//go:generate mockgen -destination mocks/mock_web_analytics_repository.go -package mocks github.com/Mailwave/mailwave/internal/domain WebAnalyticsRepository
//go:generate mockgen -destination mocks/mock_web_analytics_service.go -package mocks github.com/Mailwave/mailwave/internal/domain WebAnalyticsService
//go:generate mockgen -destination mocks/mock_geoip_resolver.go -package mocks github.com/Mailwave/mailwave/internal/domain GeoIPResolver
...
type WebGoal struct {
	...
	// GoalType is asserted by the site, not verified by Mailwave — the same page
...
// unsubscribe, one-click) and is printed into every email Mailwave sends.
...
// token in. The literal is owned by mailwave_mjml, which runs the link-rewriting
...
const WebIdentifyQueryParam = mailwave_mjml.WebIdentifyQueryParam
...
type WebAnalyticsSettings struct {
	...
	// IdentifyFromEmailLinks lets Mailwave mint an identity credential into the
...
// CanIdentifyFromEmailLinks reports whether Mailwave may mint an identity into
...
// mailwave_mjml.MatchesAllowedHost, which reads the same list to decide whether
...
func (s *WebAnalyticsSettings) MatchesAllowedDomain(hostname string) bool {
	...
	return mailwave_mjml.MatchesAllowedHost(hostname, s.AllowedDomains)
}
